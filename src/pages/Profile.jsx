...
const Profile = () => {
    ...
    const stats = [
        ...
        { 
            label: 'Auth Source', 
            value: user.provider ? (user.provider.charAt(0).toUpperCase() + user.provider.slice(1)) : 'Email', 
            icon: Shield, 
            color: user.provider === 'local' ? 'bg-blue-500/10 text-blue-600' : 'bg-green-500/10 text-green-600' 
        },
    ];
                    ...
                    <div className="text-center md:text-left space-y-1 flex-1 w-full md:w-auto">
                        ...
                        {!isEditing && (
                            <div className="space-y-2">
                                <p className="text-subtext font-medium text-sm md:text-base">{user.email}</p>
                                
                                {/* Social Provider Badge */}
                                {user.provider && user.provider !== 'local' && (
                                    <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-secondary/50 border border-border rounded-full shadow-sm hover:translate-y-[-1px] transition-all">
                                        <div className="w-4 h-4 flex items-center justify-center">
                                            ...
                                            {user.provider === 'github' && (
                                                <svg className="w-full h-full fill-maintext" viewBox="0 0 24 24">
                                            ...
                                            {user.provider === 'discord' && (
                                                <svg className="w-full h-full fill-[#5865F2]" viewBox="0 0 24 24">
