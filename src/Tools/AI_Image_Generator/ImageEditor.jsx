    ...
    <div className="fixed inset-0 z-[60] bg-black/95 flex flex-col items-center select-none touch-none">
      ...
      <div className="w-full flex items-center justify-between p-4 bg-black border-b border-white/10 z-10">
        ...
        <button
          ...
          className="p-2 bg-primary text-white rounded-lg hover:bg-primary/90 flex items-center gap-2 px-4 shadow-lg shadow-primary/20"
        >
          ...
          {activeTool === 'filter' && (
            <div className="flex gap-4">
              {FILTERS.map(f => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full bg-white/10 border-2 overflow-hidden ${filter.name === f.name ? 'border-primary' : 'border-transparent'}`}
                  >
