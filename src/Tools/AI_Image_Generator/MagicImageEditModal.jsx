...
const MagicImageEditModal = ({ isOpen, onClose, onCreditDeduction }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (error.response?.data?.error === 'Insufficient credits') {
        ...
        toast.error(
          error.response?.data?.message || error.response?.data?.error || 'Failed to edit image'
        );
      }
    } finally {
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 dark:bg-black/80 backdrop-blur-md overflow-y-auto modal-open-indicator">
          <motion.div
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-subtext font-medium">
                    Google Vertex AI Magic ⚡ Intelligent Image Editing
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 custom-scrollbar flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-2">
                  <span className="text-xs font-bold text-subtext uppercase tracking-wider">
                    Source Image
                  </span>
                  {previewUrl ? (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div
                    className={`relative w-full aspect-square rounded-2xl overflow-hidden border ${isGenerating ? 'border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'border-border'} flex items-center justify-center bg-black/5 dark:bg-white/5`}
                  >
                    {isGenerating ? (
                      <div className="flex flex-col items-center gap-4 text-amber-500 animate-in fade-in duration-500">
                        <Loader2 className="w-8 h-8 animate-spin" />
                        <p className="text-sm font-semibold animate-pulse">
                          Vertex AI is editing...
                        </p>
                      </div>
                    ) : resultImage ? (
                      <img
              ...
              <div className="flex flex-col gap-2 shrink-0">
                <label className="text-xs font-bold text-subtext uppercase tracking-wider">
                  Editing Prompt
                </label>
                ...
                <p className="text-[11px] text-subtext ml-1">
                  Be descriptive. Use phrases like "replace the sky with...", "make it look like an
                  anime", "add sunglasses".
                </p>
              </div>
            </div>
