...
const CustomVideoPlayer = ({ src, compact = false }) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative overflow-hidden bg-black/95 rounded-2xl border border-white/5 shadow-2xl group flex flex-col justify-center ${
        isFullscreen
    ...
    >
      ...
      {skipAnim === 'left' && (
        <div className="absolute left-[25%] top-1/2 -translate-y-1/2 -translate-x-1/2 z-20 pointer-events-none flex flex-col items-center animate-pulse">
          ...
          <span className="text-white font-bold text-xs sm:text-base drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
            -2s
          </span>
        </div>
      )}
      ...
      <img
        src={logo}
        alt="AISA Watermark"
        className={`absolute transition-all duration-300 pointer-events-none z-[15] opacity-60 select-none drop-shadow-2xl ${
          compact
      ...
      >
        <div
          className={`bg-[#2A2B32]/90 backdrop-blur-xl border border-white/10 ${compact ? 'rounded-lg px-2 py-2 flex items-center gap-2' : 'rounded-lg sm:rounded-xl px-2 py-2 sm:px-4 sm:py-3 flex items-center gap-2 sm:gap-4 md:gap-5'} shadow-[0_8px_32px_rgba(0,0,0,0.5)]`}
        >
          {/* Download Button */}
          <button
          ...
          >
            {isDownloading ? (
              <div
                className={`border-2 border-white/30 border-t-white rounded-full animate-spin ${compact ? 'w-3 h-3' : 'w-3 h-3 sm:w-3.5 sm:h-3.5'}`}
              />
            ) : (
          ...
          >
            {isPlaying ? (
              <Pause
                className={compact ? 'w-4 h-4 fill-current' : 'w-4 h-4 sm:w-5 sm:h-5 fill-current'}
              />
            ) : (
              <Play
                className={compact ? 'w-4 h-4 fill-current' : 'w-4 h-4 sm:w-5 sm:h-5 fill-current'}
          ...
          >
            {isMuted ? (
              <VolumeX className={compact ? 'w-3.5 h-3.5' : 'w-4 h-4 sm:w-5 sm:h-5'} />
            ) : (
              <Volume2 className={compact ? 'w-3.5 h-3.5' : 'w-4 h-4 sm:w-5 sm:h-5'} />
            )}
          </button>
          ...
          >
            {isFullscreen ? (
              <Minimize className={compact ? 'w-3.5 h-3.5' : 'w-4 h-4 sm:w-5 sm:h-5'} />
            ) : (
              <Maximize className={compact ? 'w-3.5 h-3.5' : 'w-4 h-4 sm:w-5 sm:h-5'} />
            )}
          </button>
        </div>
      </div>
