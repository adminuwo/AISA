            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-white truncate max-w-[400px]">
                      {selectedItem.name}
                    </h3>
                ...
                <button
                  onClick={() => setSelectedItem(null)}
                  className="p-2 hover:bg-white/5 rounded-full text-subtext transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 mb-8">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-subtext uppercase tracking-widest">
                    {selectedItem.isSource ? 'Pages Indexed' : 'Chunks'}
                  </p>
              ...
              {selectedItem.raw.gcsUri && (
                <div className="bg-black/40 rounded-2xl p-4 border border-white/5 mb-8">
                  <p className="text-[10px] font-black text-subtext uppercase tracking-widest mb-1">
                    Retrieval URI (GCS)
                  </p>
                ...
                <button
                  onClick={() => {
                    if (selectedItem.isSource)
                      setDeleteModal({ isOpen: true, type: 'source', id: selectedItem.id });
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          <thead>
            <tr className="bg-white/5 border-b border-white/10">
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-subtext">
                Source Name
              </th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-subtext">
                Type
              ...
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-subtext text-center">
                Chunks/Pages
              </th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-subtext">
                Status
              </th>
              <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider text-subtext text-right">
                Actions
              </th>
            </tr>
          </thead>
              ...
              >
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${item.isSource ? 'bg-indigo-500/10 text-indigo-400' : 'bg-blue-500/10 text-blue-400'}`}
                    >
                      {item.isSource ? (
                    ...
                    <div className="max-w-[250px]">
                      <p className="text-sm font-bold text-maintext truncate" title={item.name}>
                        {item.name}
                      </p>
                ...
                <td className="px-6 py-4 text-center">
                  <span className="text-sm font-black text-maintext">{item.chunks}</span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-1 transition-opacity">
                    ...
                    <button
                      onClick={() =>
                        item.isSource
                          ? setDeleteModal({ isOpen: true, type: 'source', id: item.id })
                          : setDeleteModal({ isOpen: true, type: 'doc', id: item.id })
                      }
        ...
        {filteredItems.length === 0 && (
          <div className="py-20 text-center">
            ...
            <p className="text-subtext font-medium">
              No knowledge assets found matches your criteria.
            </p>
          </div>
        )}
      </div>
      ...
      <DeleteConfirmModal
        ...
        description={
          deleteModal.type === 'source'
            ? 'Are you sure you want to delete this website source and all crawled pages? This cannot be undone.'
            : 'Are you sure you want to delete this document and all its embeddings? This cannot be undone.'
        }
      />
    </div>
  );
