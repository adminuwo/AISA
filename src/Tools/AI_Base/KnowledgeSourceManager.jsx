    ...
    <div className="mt-8">
      ...
      {sources.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-10 text-center">
          ...
          <p className="text-xs text-subtext/60 mt-2">
            Use the "URL Upload" tab to add a root domain for periodic crawling.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div className="flex items-start gap-4 flex-1">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${source.status === 'error' ? 'bg-red-500/10' : 'bg-primary/10'}`}
                  >
                    <Globe
                      className={`w-6 h-6 ${source.status === 'error' ? 'text-red-400' : 'text-primary'}`}
                    />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="font-bold text-maintext truncate max-w-md" title={source.url}>
                        {source.domain}
                      </h4>
                      <span
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:flex lg:items-center gap-4 lg:gap-8 px-4 lg:px-0 lg:border-l lg:border-white/10 lg:pl-8">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-black text-maintext">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-black text-primary">
                      {source.next_crawl_at
                        ? new Date(source.next_crawl_at).toLocaleDateString()
                        : 'Pending'}
                    </p>
                  </div>
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-black text-subtext/60">
                      D:{source.crawl_depth} P:{source.max_pages}
                    </p>
                  </div>
                </div>
