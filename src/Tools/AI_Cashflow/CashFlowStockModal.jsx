...
const CashFlowStockModal = ({ isOpen, onClose, onSelect, isDarkMode, initialStock }) => {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'Realtime chart') {
        ...
        if (socket) {
          ...
          const handleRealtime = data => {
            setTabData(prev => ({
              ...
              'Realtime chart': {
                quote: data.quote,
                intraday: prev['Realtime chart']?.intraday || [],
              },
            }));
          };
      ...
      axios
        .get(`${baseURL}/stock/intraday`, { params, headers })
        .then(res => {
          setTabData(prev => ({
            ...prev,
            'Realtime chart': { quote: prev['Realtime chart']?.quote, intraday: res.data.intraday },
          }));
          setIsLoadingTab(false);
      ...
      if (activeTab === 'News')
        promise = axios
          .get(`${baseURL}/stock/news`, { params, headers })
          .then(r => ({ news: r.data.news }));
      else if (activeTab === 'Advisory')
        promise = axios
          .get(`${baseURL}/stock/advisory`, { params, headers })
          .then(r => ({ advisory: r.data.advisory }));
      else if (activeTab === 'Research and recommendation')
        promise = axios
          .get(`${baseURL}/stock/research`, { params: { symbol: selectedStock.symbol }, headers })
          .then(r => ({ research: r.data.research }));
      ...
      if (promise) {
        promise
          .then(result => {
            ...
            const mappedTab =
              activeTab === 'News' ? 'news' : activeTab === 'Advisory' ? 'advisory' : 'research';
  ...
  const fetchGrahamAnalysis = async () => {
    ...
    try {
      ...
      setUnlockedTabs(prev =>
        prev.includes('graham-analysis') ? prev : [...prev, 'graham-analysis']
      );
    } catch (err) {
  ...
  const fetchKiyosakiAnalysis = async () => {
    ...
    try {
      ...
      setUnlockedTabs(prev =>
        prev.includes('kiyosaki-analysis') ? prev : [...prev, 'kiyosaki-analysis']
      );
    } catch (err) {
  ...
  const currencySymbol = useMemo(() => {
    ...
    if (
      selectedStock?.region === 'IN' ||
      selectedStock?.symbol.includes('.BSE') ||
      selectedStock?.symbol.includes('.NSE') ||
      selectedStock?.symbol.endsWith('.BO') ||
      selectedStock?.symbol.endsWith('.NS')
    ) {
      return '₹';
    }
    return '$';
  ...
  const isRealtimePositiveTrend = useMemo(() => {
    ...
    return (
      parseFloat(realtimeLineData[realtimeLineData.length - 1].price) >=
      parseFloat(realtimeLineData[0].price)
    );
  }, [realtimeLineData]);
  ...
  const isHistoricalPositiveTrend = useMemo(() => {
    ...
    return (
      parseFloat(historicalLineData[historicalLineData.length - 1].price) >=
      parseFloat(historicalLineData[0].price)
    );
  }, [historicalLineData]);
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-[#1a1a24] border border-black/10 dark:border-white/10 p-3 rounded-xl shadow-xl backdrop-blur-md">
          <p className="text-[10px] font-bold text-gray-400 dark:text-zinc-500 mb-1 uppercase tracking-wider">
            {label}
          </p>
  ...
  return createPortal(
    <AnimatePresence>
      {isOpen && (
        <div
          className={`fixed inset-0 z-[110000] flex items-center justify-center bg-black/40 backdrop-blur-sm ${isMaximized ? 'p-0' : 'p-0 sm:p-4'}`}
        >
          <motion.div
          ...
          >
            ...
            <div className="px-3 py-3 sm:px-6 sm:py-5 bg-[#5154ff] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-base sm:text-2xl font-black text-white leading-tight font-sans tracking-tight truncate">
                    AI CashFlow Explorer
                  </h2>
                  <p className="text-[8px] sm:text-[10px] text-white/60 font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] mt-0.5 truncate">
                    Market Intelligence Â· Real-time Analytics Â· Strategy
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                >
                  {isMaximized ? (
                    <Minimize2 className="w-5 h-5 sm:w-6 sm:h-6 transition-transform" />
                  ) : (
                    <Maximize2 className="w-5 h-5 sm:w-6 sm:h-6 transition-transform group-hover:scale-110" />
                  )}
                </button>
                <button
                  onClick={onClose}
            ...
            <div className="px-3 py-3 sm:px-8 sm:py-8 border-b border-gray-100 dark:border-white/8 flex flex-col sm:flex-row gap-3 sm:gap-6 bg-[#fdfaf5] dark:bg-[#161B2E] shrink-0">
              <div className="flex-1 sm:max-w-[320px] relative">
                <label className="block text-[10px] sm:text-[11px] font-black text-[#999] uppercase tracking-[0.1em] mb-1.5 sm:mb-2.5 ml-1">
                  Country
                </label>
                <div
                  onClick={() => setIsCountryDropdownOpen(!isCountryDropdownOpen)}
                  className={`bg-white dark:bg-[#1E2438] border ${
                    isCountryDropdownOpen
                      ? 'border-[#5154ff] ring-4 ring-[#5154ff]/10'
                ...
                >
                  <span className="text-[18px] sm:text-[22px] leading-none">
                    {COUNTRIES.find(c => c.code === selectedCountry)?.flag || '🌐'}
                  </span>
                  <span className="text-[#888] dark:text-zinc-500 font-extrabold text-[12px] sm:text-[13px]">
                    {selectedCountry}
                  </span>
                  <span className="text-[#111] dark:text-white flex-1 text-[13px] sm:text-[14px] truncate">
                    {COUNTRIES.find(c => c.code === selectedCountry)?.name || 'Unknown'}
                  </span>
                  <ChevronDown
                    className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${
                      isCountryDropdownOpen
                        ? 'rotate-180 text-[#5154ff]'
                        : 'text-[#ccc] dark:text-zinc-600 group-hover:text-[#5154ff]'
                    }`}
                  />
                </div>
                      ...
                      <div className="py-2 max-h-64 overflow-y-auto custom-scrollbar">
                        {COUNTRIES.filter(c => c.supported).map(country => (
                          ...
                          >
                            ...
                            <span className="text-[12px] font-black text-[#888] dark:text-zinc-500 w-8">
                              {country.code}
                            </span>
                            <span className="text-[14px] font-bold text-[#111] dark:text-white group-hover:text-[#5154ff] flex-1">
                              {country.name}
                            </span>
                            {selectedCountry === country.code && (
                              <Check className="w-4 h-4 text-[#5154ff]" />
                            )}
                          </button>
                        ))}
                      </div>
              ...
              <div className="flex-1 relative">
                ...
                >
                  <span className="text-[#111] dark:text-white truncate text-[12px] sm:text-[14px]">
                    {selectedStock?.symbol} - {selectedStock?.name}
                  </span>
                  <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                    {isSearching && <Loader2 className="w-4 h-4 animate-spin text-[#5154ff]" />}
                    <ChevronDown
                      className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ${isStockSelectOpen ? 'rotate-180 text-[#5154ff]' : 'text-[#ccc] dark:text-zinc-600 group-hover:text-[#5154ff]'}`}
                    />
                  </div>
                </div>
                      ...
                      <div className="py-2">
                        {searchTerm.length >= 2 && searchResults.length === 0 ? (
                          <div className="px-6 py-8 text-center text-rose-500">
                            ...
                            <p className="text-xs text-gray-500 dark:text-zinc-500 font-bold mt-1">
                          ...
                          (searchTerm.length >= 2 ? searchResults : PRESET_STOCKS).map(stock => (
                            ...
                            >
                              <div>
                                <div className="text-[14px] font-black text-[#111] dark:text-white group-hover:text-[#5154ff]">
                                  {stock.symbol}
                                </div>
                                <div className="text-[11px] text-[#888] dark:text-zinc-500 font-bold uppercase tracking-wider">
                                  {stock.name}
                                </div>
                              </div>
                              {selectedStock?.symbol === stock.symbol && (
                                <Check className="w-5 h-5 text-[#5154ff]" />
                              )}
                            </button>
                          ))
                        )}
              ...
              {isLoadingTab ? (
                ...
                <div className="h-full flex flex-col items-center justify-center gap-4 text-rose-500">
                  ...
                  <p className="text-[12px] text-[#888] dark:text-zinc-500 font-bold text-center max-w-sm mt-2">
                    If using AngelOne SmartAPI, ensure your API_KEY and AUTHORIZATION_TOKEN are
                    correctly configured in your Backend .env file.
                  </p>
                </div>
              ) : !tabData[activeTab] ? (
                    ...
                    <div className="space-y-4 sm:space-y-8">
                      <div className="grid grid-cols-3 gap-2 sm:grid-cols-3 sm:gap-6">
                        <div className="bg-[#fcf8f0] dark:bg-[#1E2438] rounded-[12px] sm:rounded-[20px] p-3 sm:p-6 border border-[#f0ebe0] dark:border-white/8 shadow-sm relative overflow-hidden group hover:shadow-md dark:hover:border-[#5154ff]/30 transition-all">
                          ...
                          <p className="text-lg sm:text-4xl font-black text-[#111] dark:text-white tracking-tight">
                            {tabData['Realtime chart'].quote?.price
                              ? `${currencySymbol}${parseFloat(tabData['Realtime chart'].quote.price).toLocaleString()}`
                              : currencySymbol + '---'}
                          </p>
                          <div
                            className={`flex items-center gap-1 sm:gap-1.5 mt-1 sm:mt-3 text-[10px] sm:text-[13px] font-bold ${parseFloat(tabData['Realtime chart'].quote?.change) >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}
                          >
                            <TrendingUp
                              className={`w-3 h-3 sm:w-4 sm:h-4 ${parseFloat(tabData['Realtime chart'].quote?.change) < 0 ? 'rotate-180' : ''}`}
                            />
                            {tabData['Realtime chart'].quote?.changePercent || '0.00%'}
                          </div>
                        </div>
                        <div className="bg-[#fcf8f0] dark:bg-[#1E2438] rounded-[12px] sm:rounded-[20px] p-3 sm:p-6 border border-[#f0ebe0] dark:border-white/8 shadow-sm relative overflow-hidden group hover:shadow-md dark:hover:border-[#5154ff]/30 transition-all">
                          <p className="text-[8px] sm:text-[11px] font-black text-[#aaa] dark:text-zinc-500 uppercase tracking-widest mb-1 sm:mb-2">
                            Day high
                          </p>
                          <p className="text-lg sm:text-4xl font-black text-[#111] dark:text-white tracking-tight">
                            {tabData['Realtime chart'].quote?.high
                              ? `${currencySymbol}${parseFloat(tabData['Realtime chart'].quote.high).toLocaleString()}`
                              : currencySymbol + '---'}
                          </p>
                        ...
                        <div className="bg-[#fcf8f0] dark:bg-[#1E2438] rounded-[12px] sm:rounded-[20px] p-3 sm:p-6 border border-[#f0ebe0] dark:border-white/8 shadow-sm relative overflow-hidden group hover:shadow-md dark:hover:border-[#5154ff]/30 transition-all">
                          ...
                          <p className="text-lg sm:text-4xl font-black text-[#111] dark:text-white tracking-tight">
                            {tabData['Realtime chart'].quote?.volume
                              ? (parseFloat(tabData['Realtime chart'].quote.volume) / 1000).toFixed(
                                  1
                                ) + 'k'
                              : '---'}
                          </p>
                          <div className="mt-2 sm:mt-4 flex items-center gap-1 sm:gap-2 text-[8px] sm:text-[11px] font-black text-[#5154ff] uppercase tracking-wider">
                            <span className="hidden sm:inline">AngelOne Live Data</span>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-8 gap-3">
                          <div>
                            <h3 className="text-sm sm:text-lg font-black text-[#111] dark:text-white flex items-center gap-2">
                              ...
                              <Sparkles className="w-4 h-4 sm:w-5 sm:h-5 text-[#5154ff]" />
                            </h3>
                            <p className="text-[9px] sm:text-[11px] font-bold text-[#aaa] dark:text-zinc-500 uppercase tracking-[0.15em] mt-1 mb-2 sm:mb-3">
                          ...
                          {tabData['Realtime chart'].quote?.price && (
                            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                              <button
                                ...
                                className="flex items-center gap-1.5 sm:gap-2 px-2.5 py-1.5 sm:px-4 sm:py-2 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/8 text-gray-700 dark:text-zinc-300 text-[9px] sm:text-[11px] font-black uppercase tracking-wider rounded-lg transition-colors sm:mr-4 border border-gray-200 dark:border-zinc-700 shadow-sm"
                              >
                              ...
                              <div className="flex flex-col items-center justify-center px-3 py-1 sm:px-5 sm:py-1.5 bg-[#fffafb] dark:bg-red-500/10 border border-[#f23645] rounded-[6px] min-w-[70px] sm:min-w-[100px] shadow-sm cursor-pointer hover:bg-red-50 dark:hover:bg-red-500/20 transition-colors">
                                <span className="text-[14px] sm:text-[18px] font-semibold text-[#f23645] leading-tight">
                                  {parseFloat(
                                    tabData['Realtime chart'].quote.price - 0.25
                                  ).toLocaleString('en-IN', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                  })}
                                </span>
                                <span className="text-[10px] sm:text-[12px] font-medium text-[#f23645] uppercase tracking-wide">
                                  SELL
                                </span>
                              </div>
                              <span className="text-[11px] sm:text-[13px] font-medium text-[#333] dark:text-zinc-400">
                                0.50
                              </span>
                              <div className="flex flex-col items-center justify-center px-3 py-1 sm:px-5 sm:py-1.5 bg-[#f8fbff] dark:bg-blue-500/10 border border-[#2962ff] rounded-[6px] min-w-[70px] sm:min-w-[100px] shadow-sm cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-500/20 transition-colors">
                                ...
                                <span className="text-[10px] sm:text-[12px] font-medium text-[#2962ff] uppercase tracking-wide">
                                  BUY
                                </span>
                              </div>
                            </div>
                        ...
                        tabData['News'].news?.map((item, idx) => (
                          ...
                          >
                            <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                              <span className="text-[9px] sm:text-[10px] font-black text-[#5154ff] bg-white dark:bg-white/5 border border-[#5154ff]/10 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg uppercase tracking-wider">
                                {item.source || 'Finance'}
                              </span>
                              <span className="text-[9px] sm:text-[10px] font-bold text-[#888] dark:text-zinc-500 uppercase tracking-wider">
                                {getRelativeTime(item.time_published)}
                              </span>
                            </div>
                            <h4 className="text-[14px] sm:text-[17px] font-black text-[#111] dark:text-white mb-2 sm:mb-3 group-hover:text-[#5154ff] transition-colors leading-snug">
                              {item.title}
                            </h4>
                            <p className="text-[11px] sm:text-[13px] text-[#777] dark:text-zinc-400 mb-3 sm:mb-4 line-clamp-2 leading-relaxed">
                              {item.summary}
                            </p>
                            <div className="flex items-center justify-between">
                              <span
                                className={`text-[8px] sm:text-[10px] font-black px-2 py-1 sm:px-3 sm:py-1.5 rounded-full uppercase tracking-[0.05em] ${(item.overall_sentiment_label || 'Neutral').includes('Bullish') ? 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600' : (item.overall_sentiment_label || 'Neutral').includes('Bearish') ? 'bg-rose-50 dark:bg-rose-500/20 text-rose-600' : 'bg-white dark:bg-white/5 border border-gray-200 dark:border-zinc-700 text-[#999] dark:text-zinc-500'}`}
                              >
                              ...
                              <a
                                ...
                                className="p-2 sm:p-2.5 bg-white dark:bg-white/5 rounded-xl text-[#5154ff] hover:bg-[#5154ff] hover:text-white transition-all shadow-sm"
                              >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-8 gap-3">
                        <div>
                          <h3 className="text-sm sm:text-xl font-black text-[#111] dark:text-white flex items-center gap-2 sm:gap-3">
                            ...
                            <BarChart3 className="w-4 h-4 sm:w-6 sm:h-6 text-[#5154ff]" />
                          </h3>
                          <p className="text-[9px] sm:text-[12px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-[0.15em] sm:tracking-[0.2em] mt-1">
                            Interactive TradingView Historical Record
                          </p>
                        </div>
                        <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                          ...
                          >
                            <Activity
                              className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${chartType === 'tradingview' ? 'animate-pulse' : ''}`}
                            />{' '}
                            TradingView
                          </button>
                          <button
                            onClick={() => setFullScreenChart('historical')}
                            className="flex items-center gap-1.5 sm:gap-2 px-2.5 py-1.5 sm:px-4 sm:py-2 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/8 text-gray-700 dark:text-zinc-300 text-[9px] sm:text-[11px] font-black uppercase tracking-wider rounded-lg transition-colors border border-gray-200 dark:border-zinc-700 shadow-sm"
                          >
                            <Maximize className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> Pro View
                          </button>
                        </div>
                      ...
                      <div className="h-[280px] sm:h-[550px] w-full pt-2 sm:pt-4 relative rounded-xl overflow-hidden shadow-inner border border-black/5 dark:border-white/5">
                        {chartType === 'tradingview' ? (
                          <TradingViewWidget
                            symbol={selectedStock?.symbol}
                            interval="D"
                            containerId="tv_chart_historical"
                            isDarkMode={isDarkMode}
                          />
                        ) : (
                              ...
                              <AreaChart data={historicalLineData}>
                                <defs>
                                  ...
                                  >
                                    <stop
                                      offset="5%"
                                      stopColor={isHistoricalPositiveTrend ? '#10b981' : '#e11d48'}
                                      stopOpacity={0.2}
                                    />
                                    <stop
                                      offset="95%"
                    ...
                    <div className="h-full flex items-center justify-center py-2 overflow-hidden">
                      <motion.div
                        ...
                        className={`w-full max-w-3xl border border-black/5 dark:border-white/5 rounded-[18px] sm:rounded-[28px] p-4 sm:p-6 shadow-lg relative overflow-hidden flex flex-col md:flex-row items-center gap-4 sm:gap-6
                                     ${
                                       tabData['Advisory'].advisory.verdict === 'BUY'
                                         ? 'bg-gradient-to-br from-emerald-50/70 to-teal-50/70 dark:from-emerald-900/20 dark:to-teal-900/20'
                                         : tabData['Advisory'].advisory.verdict === 'SELL'
                                           ? 'bg-gradient-to-br from-rose-50/70 to-orange-50/70 dark:from-rose-900/20 dark:to-orange-900/20'
                      ...
                      >
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-1 sm:space-y-2">
                          <div
                            className={`p-3 sm:p-4 rounded-full bg-white/60 dark:bg-black/20 shadow-sm 
                                        ${
                                          tabData['Advisory'].advisory.verdict === 'BUY'
                                            ? 'text-emerald-500'
                                            : tabData['Advisory'].advisory.verdict === 'SELL'
                          ...
                          <div>
                            <h3 className="text-[10px] sm:text-[12px] font-black text-[#888] uppercase tracking-[0.2em] sm:tracking-[0.3em] mb-0.5">
                              System Signal
                            </h3>
                            <p
                        ...
                        <div className="w-full md:w-[320px] grid grid-cols-2 gap-2 sm:gap-3">
                          <div className="bg-white/80 dark:bg-[#1E2438]/80 p-2.5 sm:p-3.5 rounded-[14px] sm:rounded-[20px] border border-white dark:border-white/8 shadow-sm hover:shadow-md transition-shadow">
                            <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-[#aaa] dark:text-zinc-500 mb-0.5">
                              RSI
                            </p>
                            <p className="text-base sm:text-xl font-black text-[#111] dark:text-white">
                              {tabData['Advisory'].advisory.indicators?.RSI}
                            </p>
                          </div>
                          <div className="bg-white/80 dark:bg-[#1E2438]/80 p-2.5 sm:p-3.5 rounded-[14px] sm:rounded-[20px] border border-white dark:border-white/8 shadow-sm hover:shadow-md transition-shadow">
                            <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-[#aaa] dark:text-zinc-500 mb-0.5">
                            ...
                            <p className="text-base sm:text-xl font-black text-[#111] dark:text-white">
                              {tabData['Advisory'].advisory.indicators?.MACD}
                            </p>
                          </div>
                          <div className="bg-white/80 dark:bg-[#1E2438]/80 p-2.5 sm:p-3.5 rounded-[14px] sm:rounded-[20px] border border-white dark:border-white/8 shadow-sm hover:shadow-md transition-shadow">
                            <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-[#aaa] dark:text-zinc-500 mb-0.5">
                            ...
                            <p
                              className={`text-xs sm:text-sm font-black ${tabData['Advisory'].advisory.indicators?.Ichimoku === 'Bullish' ? 'text-emerald-600' : tabData['Advisory'].advisory.indicators?.Ichimoku === 'Bearish' ? 'text-rose-600' : 'text-[#111] dark:text-white'}`}
                            >
                          ...
                          <div className="bg-white/80 dark:bg-[#1E2438]/80 p-2.5 sm:p-3.5 rounded-[14px] sm:rounded-[20px] border border-white dark:border-white/8 shadow-sm hover:shadow-md transition-shadow">
                            ...
                            <p className="text-base sm:text-xl font-black text-[#111] dark:text-white">
                              {tabData['Advisory'].advisory.indicators?.SMA}
                            </p>
                          </div>
                          <div className="col-span-2 bg-white/80 dark:bg-[#1E2438]/80 p-2.5 sm:p-3 rounded-[14px] sm:rounded-[20px] border border-white dark:border-white/8 shadow-sm flex justify-between items-center px-4 sm:px-6">
                            <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-[#aaa] dark:text-zinc-500">
                              Fibonacci Level
                            </p>
                            <p className="text-base sm:text-xl font-black text-[#5154ff]">
                              ₹{tabData['Advisory'].advisory.indicators?.Fibonacci || '---'}
                            </p>
                          </div>
                        </div>
                      </motion.div>
                    ...
                    <div className="space-y-6">
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            showGrahamPanel ? setShowGrahamPanel(false) : fetchGrahamAnalysis()
                          }
                        ...
                        >
                          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                            ...
                            <div className="text-left min-w-0">
                              <p className="text-[9px] sm:text-[11px] font-black uppercase tracking-[0.15em] sm:tracking-[0.25em] text-amber-400/70 mb-0.5 truncate">
                                Value Investing Analysis
                              </p>
                              <p className="text-sm sm:text-lg font-black text-amber-100 truncate">
                                Benjamin Graham <span className="text-amber-400">Perspective</span>
                              </p>
                              <p className="text-[9px] sm:text-[11px] text-amber-300/60 font-semibold mt-0.5 truncate">
                          ...
                          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                            {isGrahamLoading ? (
                              ...
                              <ChevronRight
                                className={`w-4 h-4 sm:w-5 sm:h-5 text-amber-400 transition-transform duration-300 group-hover:translate-x-1 ${showGrahamPanel ? 'rotate-90' : ''}`}
                              />
                            )}
                          </div>
                        </button>
                              ...
                              {isGrahamLoading ? (
                                <div className="mt-4 flex flex-col items-center justify-center gap-4 py-16 bg-amber-950/30 rounded-[20px] border border-amber-900/40">
                                  ...
                                  <p className="text-sm font-black text-amber-400 uppercase tracking-widest animate-pulse">
                                    Consulting The Intelligent Investor...
                                  </p>
                                </div>
                              ) : grahamData ? (
                                <div className="mt-4 bg-gradient-to-br from-amber-950/20 to-stone-900/10 border border-amber-900/30 rounded-[16px] sm:rounded-[24px] p-4 sm:p-8 space-y-4 sm:space-y-6">
                                  ...
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 sm:pb-5 border-b border-amber-900/30 gap-3">
                                    <div className="flex items-center gap-2 sm:gap-3">
                                      ...
                                      <div>
                                        <h3 className="text-sm sm:text-lg font-black text-stone-800 dark:text-white">
                                          Graham&apos;s Verdict
                                        </h3>
                                        <p className="text-[9px] sm:text-[11px] text-stone-500 dark:text-zinc-500 font-bold uppercase tracking-wider">
                                          {grahamData.source}
                                        </p>
                                      </div>
                                    </div>
                                    <div
                                      className={`px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest w-fit
                                                      ${
                                                        grahamData.graham_verdict === 'BUY'
                                                          ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800'
                                                          : grahamData.graham_verdict === 'AVOID'
                                                            ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800'
                                  ...
                                  <blockquote className="relative pl-6 border-l-4 border-amber-400 italic text-stone-700 dark:text-zinc-300 text-[13px] sm:text-[15px] font-semibold leading-relaxed">
                                    <span className="absolute -left-3 -top-2 text-4xl sm:text-5xl text-amber-300/50 font-serif leading-none">
                                      &ldquo;
                                    </span>
                                  ...
                                  <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-[16px] p-4 flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm font-bold text-amber-900 dark:text-amber-100">
                                        {grahamData.key_principle_applied}
                                      </p>
                                    </div>
                                  </div>
                                    ...
                                    {[
                                      {
                                        ...
                                        value: grahamData.margin_of_safety,
                                        color:
                                          'text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 border-emerald-100 dark:border-emerald-900',
                                      },
                                      {
                                        label: 'Intrinsic Value',
                                        value: grahamData.intrinsic_value_note,
                                        color:
                                          'text-blue-600 bg-blue-50 dark:bg-blue-950/30 border-blue-100 dark:border-blue-900',
                                      },
                                      {
                                        label: 'For Defensive Investor',
                                        value: grahamData.defensive_investor,
                                        color:
                                          'text-violet-600 bg-violet-50 dark:bg-violet-950/30 border-violet-100 dark:border-violet-900',
                                      },
                                      {
                                        label: 'For Enterprising Investor',
                                        value: grahamData.enterprising_investor,
                                        color:
                                      ...
                                      <div
                                        key={i}
                                        className={`rounded-[16px] border p-4 sm:p-5 ${item.color.split(' ').slice(1).join(' ')}`}
                                      >
                                        <p
                                          className={`text-[10px] font-black uppercase tracking-widest mb-2 ${item.color.split(' ')[0]}`}
                                        >
                                          {item.label}
                                        </p>
                                        <p className="text-sm font-semibold text-stone-700 dark:text-zinc-300 leading-relaxed">
                                          {item.value}
                                        </p>
                                      </div>
                                    ))}
                                  ...
                                  <div className="bg-stone-100 dark:bg-white/5/50 border border-stone-200 dark:border-zinc-700 rounded-[16px] p-5">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-stone-500 dark:text-zinc-500 mb-2">
                                      Graham Number &amp; Valuation
                                    </p>
                                    <p className="text-sm font-semibold text-stone-700 dark:text-zinc-300 leading-relaxed">
                                      {grahamData.graham_number_note}
                                    </p>
                                  </div>
                                  ...
                                  <div className="bg-gradient-to-r from-amber-900/10 to-stone-800/5 dark:from-amber-900/20 dark:to-zinc-900/20 border border-amber-800/20 dark:border-amber-900 rounded-[20px] p-6">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-amber-700 dark:text-amber-400 mb-3">
                                      Graham&apos;s Final Advice
                                    </p>
                                    <p className="text-[15px] font-semibold text-stone-800 dark:text-white leading-relaxed">
                                      {grahamData.final_advice}
                                    </p>
                                  </div>
                                  ...
                                  <div className="flex items-center gap-2 text-[11px] text-stone-400 dark:text-zinc-600 font-bold pt-2 border-t border-stone-200 dark:border-white/8">
                                    ...
                                    Source: {grahamData.source} &middot;{' '}
                                    {grahamData.rag_used
                                      ? 'ðŸ“š Knowledge Base Retrieved'
                                      : 'ðŸ¤– AI Generated'}
                                  </div>
                                </div>
                              ) : null}
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                            ...
                            <div className="text-left min-w-0">
                              <p className="text-[9px] sm:text-[11px] font-black uppercase tracking-[0.15em] sm:tracking-[0.25em] text-blue-400/70 mb-0.5 truncate">
                                Finance IQ & Cashflow
                              </p>
                              <p className="text-sm sm:text-lg font-black text-blue-100 truncate">
                                Robert Kiyosaki <span className="text-blue-400">Perspective</span>
                              </p>
                              <p className="text-[9px] sm:text-[11px] text-blue-300/60 font-semibold mt-0.5 truncate">
                                Based on &quot;Rich Dad Poor Dad&quot;
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                            {isKiyosakiLoading ? (
                              ...
                              <ChevronRight
                                className={`w-4 h-4 sm:w-5 sm:h-5 text-blue-400 transition-transform duration-300 group-hover:translate-x-1 ${showKiyosakiPanel ? 'rotate-90' : ''}`}
                              />
                            )}
                          </div>
                        </button>
                              ...
                              {isKiyosakiLoading ? (
                                <div className="mt-4 flex flex-col items-center justify-center gap-4 py-16 bg-blue-950/30 rounded-[20px] border border-blue-900/40">
                                  ...
                                  <p className="text-sm font-black text-blue-400 uppercase tracking-widest animate-pulse">
                                    Building Financial IQ with Rich Dad...
                                  </p>
                                </div>
                              ) : kiyosakiData ? (
                                <div className="mt-4 bg-gradient-to-br from-blue-950/20 to-indigo-900/10 border border-blue-900/30 rounded-[16px] sm:rounded-[24px] p-4 sm:p-8 space-y-4 sm:space-y-6">
                                  ...
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 sm:pb-5 border-b border-blue-900/30 gap-3">
                                    ...
                                    <div
                                      className={`px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest w-fit
                                                      ${
                                                        kiyosakiData.kiyosaki_verdict === 'BUY'
                                                          ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800'
                                                   ...
                                                   `}
                                    >
                                      {kiyosakiData.kiyosaki_verdict === 'AVOID'
                                        ? 'â›” AVOID'
                                        : kiyosakiData.kiyosaki_verdict === 'BUY'
                                          ? 'âœ… BUY'
                                          : 'â¸ HOLD'}
                                  ...
                                  <blockquote className="relative pl-6 border-l-4 border-blue-400 italic text-stone-700 dark:text-zinc-300 text-[13px] sm:text-[15px] font-semibold leading-relaxed">
                                    <span className="absolute -left-3 -top-2 text-4xl sm:text-5xl text-blue-300/50 font-serif leading-none">
                                      &ldquo;
                                    </span>
                                  ...
                                  <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-[16px] p-4 flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm font-bold text-blue-900 dark:text-blue-100">
                                        {kiyosakiData.financial_literacy_tip}
                                      </p>
                                    </div>
                                  </div>
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {[
                                      {
                                        label: 'Cashflow Perspective',
                                        value: kiyosakiData.cashflow_perspective,
                                        color:
                                          'text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 border-emerald-100 dark:border-emerald-900',
                                      },
                                      {
                                        label: 'Asset vs Liability',
                                        value: kiyosakiData.asset_vs_liability,
                                        color:
                                          'text-indigo-600 bg-indigo-50 dark:bg-indigo-950/30 border-indigo-100 dark:border-indigo-900',
                                      },
                                      {
                                        label: 'Risk Assessment',
                                        value: kiyosakiData.risk_assessment,
                                        color:
                                          'text-rose-600 bg-rose-50 dark:bg-rose-950/30 border-rose-100 dark:border-rose-900',
                                      },
                                      {
                                        label: 'Rich Dad Advice',
                                        value: kiyosakiData.rich_dad_advice,
                                        color:
                                          'text-blue-600 bg-blue-50 dark:bg-blue-950/30 border-blue-100 dark:border-blue-900',
                                      },
                                    ].map((item, i) => (
                                      <div
                                        key={i}
                                        className={`rounded-[16px] border p-5 ${item.color.split(' ').slice(1).join(' ')}`}
                                      >
                                        <p
                                          className={`text-[10px] font-black uppercase tracking-widest mb-2 ${item.color.split(' ')[0]}`}
                                        >
                                  ...
                                  <div className="flex items-center gap-2 text-[11px] text-stone-400 dark:text-zinc-600 font-bold pt-2 border-t border-blue-200 dark:border-white/8">
                                    ...
                                    Source: {kiyosakiData.source} &middot;{' '}
                                    {kiyosakiData.rag_used
                                      ? 'ðŸ“š Knowledge Base Retrieved'
                                      : 'ðŸ¤– AI Generated'}
                                  </div>
                                </div>
                        ...
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-gray-100 dark:border-white/8 gap-2">
                            ...
                            <div className="flex items-center gap-2 px-2 py-0.5 sm:px-3 sm:py-1 bg-black/5 dark:bg-white/5 rounded-lg text-[8px] sm:text-[10px] font-bold text-[#555] dark:text-zinc-500 uppercase tracking-wider w-fit">
                              <Activity className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Live Analysis{' '}
                              <span className="bg-[#5154ff] text-white px-1 py-0.5 sm:px-1.5 rounded-[4px] ml-1">
                                +4
                              </span>
                            </div>
                          </div>
                          <ul className="space-y-3 sm:space-y-4">
                            {[
                              ...
                              <li
                                key={idx}
                                className="text-[12px] sm:text-[16px] text-[#333] dark:text-zinc-300 leading-relaxed relative pl-5 sm:pl-6 before:content-[''] before:absolute before:left-0 before:top-[8px] sm:before:top-[10px] before:w-1.5 before:h-1.5 before:bg-[#111] dark:before:bg-white before:rounded-full"
                              >
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-8 pb-3 sm:pb-4 border-b border-gray-100 dark:border-white/8 gap-3">
                            <div>
                              <h3 className="text-base sm:text-2xl font-black text-[#111] dark:text-white flex items-center gap-2 sm:gap-3">
                                ...
                                <Activity className="w-4 h-4 sm:w-6 sm:h-6 text-[#5154ff]" />
                              </h3>
                              <p className="text-[9px] sm:text-[12px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-[0.15em] sm:tracking-[0.2em] mt-1">
                                AI-Powered Research &amp; Recommendation for {selectedStock?.name}
                              </p>
                            </div>
                            <div
                              className={`px-3 py-1.5 sm:px-5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-black uppercase tracking-widest w-fit
                          ...
                          <div className="prose prose-slate dark:prose-invert max-w-none">
                            <ReactMarkdown
                              components={{
                                p: ({ node, ...props }) => (
                                  <p
                                    className="text-[13px] sm:text-[16px] text-[#333] dark:text-zinc-300 font-semibold mb-3 sm:mb-4 leading-relaxed"
                                    {...props}
                                  />
                                ),
                                li: ({ node, ...props }) => (
                                  <li
                                    className="text-[13px] sm:text-[16px] font-bold text-[#111] dark:text-white mb-2 sm:mb-3 list-none pl-6 sm:pl-8 relative before:content-[''] before:absolute before:left-0 before:top-[10px] sm:before:top-[12px] before:w-2 before:h-2 sm:before:w-2.5 sm:before:h-2.5 before:bg-[#5154ff] before:rounded-full before:opacity-20"
                                    {...props}
                                  />
                                ),
                              }}
              ...
              >
                <div className="flex items-center justify-between px-3 py-2 sm:px-6 sm:py-4 bg-white dark:bg-[#161B2E] border-b border-gray-100 dark:border-white/8 shadow-sm shrink-0">
                  ...
                  <button
                    ...
                    className="p-2 sm:p-2.5 bg-gray-100 dark:bg-white/5 rounded-full hover:bg-rose-500 hover:text-white text-gray-500 dark:text-zinc-400 transition-all shadow-sm active:scale-95 shrink-0"
                  >
