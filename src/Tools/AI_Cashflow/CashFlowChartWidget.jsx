  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-[#1a1a24] border border-black/10 dark:border-white/10 p-3 rounded-xl shadow-xl backdrop-blur-md">
        ...
        <p className="text-lg font-black text-maintext">${Number(payload[0].value).toFixed(2)}</p>
      </div>
    );
  }
  return null;
        ...
        <div className="bg-white/50 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-3xl p-5 overflow-hidden backdrop-blur-xl relative group">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <div
                className={`p-2 rounded-xl ${isPositiveTrend ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'}`}
              >
                {isPositiveTrend ? <TrendingUp size={18} /> : <TrendingDown size={18} />}
              </div>
              <h4 className="text-sm font-black text-maintext uppercase tracking-wider">
                30-Day Price Action
              </h4>
            </div>
            {overview?.marketCap && (
              ...
              <AreaChart data={chartData}>
                <defs>
                  <linearGradient id="colorPrice" x1="0" y1="0" x2="0" y2="1">
                    <stop
                      offset="5%"
                      stopColor={isPositiveTrend ? '#10b981' : '#e11d48'}
                      stopOpacity={0.3}
                    />
                    <stop
                      offset="95%"
          ...
          <div className="bg-white/50 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-3xl p-5 backdrop-blur-xl">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-2">
              {sentimentData.map((entry, index) => (
                <div
                  key={index}
                  className="flex items-center gap-1.5 text-[10px] font-bold text-subtext uppercase"
                >
                  <div
                    className="w-2.5 h-2.5 rounded-full"
                    style={{ backgroundColor: COLORS[entry.name] || '#3b82f6' }}
                  />
        ...
        {overview && Object.keys(overview).length > 2 && (
          <div className="bg-white/50 dark:bg-black/20 border border-black/5 dark:border-white/5 rounded-3xl p-5 backdrop-blur-xl flex flex-col justify-center">
            ...
            <div className="grid grid-cols-2 gap-y-4 gap-x-2">
              <div>
                <p className="text-[10px] font-bold text-subtext uppercase tracking-widest opacity-70 mb-0.5">
                  P/E Ratio
                </p>
