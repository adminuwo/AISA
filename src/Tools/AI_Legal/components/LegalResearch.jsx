...
const LegalResearch = ({ currentCase, onBack, theme, allProjects = [], onUpdateCase }) => {
  ...
  const handleSearchChange = text => {
    ...
    if (lowerText.includes('302') || lowerText.includes('murder')) {
      ...
      setDetectedKeyword({
        type: 'Section',
        name: 'IPC Section 420 (Cheating & Delivery of Property)',
      });
    } else if (
      ...
      setDetectedKeyword({
        type: 'Article',
        name: 'Constitution Article 21 (Protection of Life & Liberty)',
      });
    } else if (lowerText.includes('376') || lowerText.includes('rape')) {
      setDetectedKeyword({
        type: 'Section',
        name: 'IPC Section 376 (Punishment for Sexual Assault)',
      });
    } else if (lowerText.includes('65b') || lowerText.includes('electronic')) {
      setDetectedKeyword({
        type: 'Section',
        name: 'Evidence Act Section 65B (Electronic Evidence)',
      });
    } else {
  ...
  const handleAttachCitations = async () => {
    const activeCase =
      allProjects.find(p => p._id === (linkedCaseId || currentCase?._id)) || currentCase;
    ...
    for (const line of lines) {
      if (
        line.toLowerCase().includes('recommended citations') ||
        line.toLowerCase().includes('citations for court')
      ) {
  ...
  const stats = activeResearch
    ? activeResearch.stats
    : { precedentsCount: '--', primaryAct: '--', confidenceRate: '--' };
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-3xl p-5 shadow-sm flex flex-col items-center justify-center text-center">
              ...
              <span className="text-lg font-black text-pink-500 mt-2">
          ...
          <div className="bg-white dark:bg-[#1A2540] rounded-3xl p-6 shadow-sm">
            <h3 className="text-xs font-black tracking-widest text-slate-400 uppercase mb-4 font-bold">
              RESEARCH DIRECTORY CLASSIFICATIONS
            </h3>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      runPrecedentDiscovery({
                        searchQuery: `Find landmarks relating to ${c.title}`,
                      });
                    }}
                  ...
                  >
                    ...
                    <h4 className="text-xs font-bold text-slate-800 dark:text-white mt-2 line-clamp-1">
                      {c.title}
                    </h4>
          ...
          <div className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-md space-y-5">
            <div className="flex items-center gap-2 border-b border-slate-100 dark:border-white/5 pb-3">
              ...
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-700 dark:text-slate-300">
                EXPLORE ACTS, RULES & CASE PRECEDENTS
              </h3>
            </div>
            ...
            {detectedKeyword && (
              <div className="flex items-center gap-2 px-4 py-2.5 bg-indigo-50 dark:bg-indigo-950/20 border border-indigo-200/30 rounded-xl text-xs font-semibold text-indigo-700 dark:text-indigo-400">
                ...
                <span>
                  Detected Target: <strong>{detectedKeyword.name}</strong>
                </span>
              </div>
            )}
          ...
          {isAuditing && (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-indigo-500 animate-pulse">
                Running statutory precedence query...
              </span>
            </div>
          )}
          ...
          {researchResult && (
            <div
              ref={scrollRef}
              className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-3xl p-6 shadow-md space-y-6"
            >
              ...
              <div
                className={`prose dark:prose-invert max-w-none text-xs sm:text-sm whitespace-pre-wrap select-text leading-relaxed text-slate-800 dark:text-slate-200 transition-opacity duration-200 ${isTranslating ? 'opacity-50' : 'opacity-100'}`}
              >
            ...
            <div className="flex-1 overflow-y-auto mt-4 space-y-3 custom-scrollbar">
              {historyData
                ...
                .map(item => (
                  <div
                    key={item.id}
                    className="flex justify-between items-start p-4 bg-slate-50 dark:bg-[#1A2540] border border-slate-200/50 dark:border-white/5 rounded-2xl shadow-sm hover:shadow-md transition-all"
                  >
                    <button
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <span className="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950/20 text-[9px] font-bold text-indigo-600 rounded-md">
                          Act: {item.stats.primaryAct}
                        </span>
                        <span className="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950/20 text-[9px] font-bold text-emerald-600 rounded-md">
                          Precedents: {item.stats.precedentsCount} found
                        </span>
                      </div>
                    </button>
                    <button
