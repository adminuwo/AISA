...
const CaseIntelligencePanel = ({ isOpen, onClose, currentCase, onUpdate, onUseInArgument }) => {
  ...
  const strengthScore =
    rawStrength > 0 && rawStrength <= 1 ? Math.round(rawStrength * 100) : rawStrength;
  const winProbability =
    rawWinProb > 0 && rawWinProb <= 1 ? Math.round(rawWinProb * 100) : rawWinProb;
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-subtext mt-2">
            {tLegal('caseStrength')}
          </span>
        </div>
        ...
        <div className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            <PieChart width={96} height={96}>
              ...
              >
                {winProbData.map((entry, index) => (
                  <Cell key={`w-${index}`} fill={WIN_COLORS[index % WIN_COLORS.length]} />
                ))}
              </Pie>
            </PieChart>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mt-2">
            {tLegal('winProbability')}
          </span>
        </div>
      </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center justify-between mb-2 px-1">
        <div>
          <h3 className="text-lg font-black text-slate-800 dark:text-white">
            {tLegal('commDashboard')}
          </h3>
          <p className="text-[10px] font-bold text-subtext uppercase tracking-widest mt-1">
            {tLegal('trackInteractions')}
          </p>
        </div>
        <button
          onClick={() =>
            setCaseData({
              ...caseData,
              communicationLogs: [
                { type: 'Note', date: new Date().toISOString(), summary: '' },
                ...(caseData.communicationLogs || []),
              ],
            })
          }
      ...
      <div className="space-y-4">
        {(caseData.communicationLogs || []).map((log, i) => (
          <div
            key={i}
            className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group relative overflow-hidden"
          >
            <div
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() =>
                  setCaseData({
                    ...caseData,
                    communicationLogs: caseData.communicationLogs.filter((_, idx) => idx !== i),
                  })
                }
        ...
        {(!caseData.communicationLogs || caseData.communicationLogs.length === 0) && (
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-zinc-900/30 rounded-3xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
            ...
            <p className="text-[11px] text-subtext mt-2 max-w-[240px] text-center font-medium">
              {tLegal('startTrackingComm')}
            </p>
          </div>
        )}
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={() =>
                setCaseData({
                  ...caseData,
                  lawyers: [...(caseData.lawyers || []), { name: '', role: '' }],
                })
              }
              ...
              <div key={idx} className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setCaseData({
                      ...caseData,
                      lawyers: caseData.lawyers.filter((_, i) => i !== idx),
                    })
                  }
      ...
      <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base sm:text-lg font-black text-slate-800 dark:text-white leading-tight">
                {tLegal('courtSchedule')}
              </h3>
              <p className="text-[9px] sm:text-[10px] font-bold text-subtext uppercase tracking-widest mt-0.5 sm:mt-1">
        ...
        <div className="space-y-3">
          <h4 className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-indigo-500 ml-1 opacity-80">
            {upcoming.length > 1
              ? tLegal('upcomingHearingsDetail')
              : tLegal('upcomingHearingDetail')}
          </h4>
          ...
          {upcoming.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              {upcoming.map((h, i) => (
                <div
                  key={`up-${i}`}
                  className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-2xl p-4 sm:px-5 sm:py-4 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] dark:shadow-none hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-all relative group"
                >
                  {upcoming.length > 1 && (
            ...
            <div className="flex flex-col items-center justify-center min-h-[240px] sm:min-h-[280px] bg-slate-50/50 dark:bg-zinc-900/40 rounded-3xl border-2 border-dashed border-slate-200 dark:border-zinc-800/50 group transition-all">
              ...
              <p className="text-[11px] font-black text-subtext uppercase tracking-[0.2em] text-center px-8">
                {tLegal('noHearingsScheduled')}
              </p>
              <p className="text-[9px] text-subtext/50 font-bold uppercase tracking-widest mt-2">
                Manage all court dates and appearances here
              </p>
            </div>
          )}
        </div>
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2 sm:gap-3">
              {past.map((h, i) => (
                <div
                  key={`past-${i}`}
                  className="bg-slate-50/50 dark:bg-black/10 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-slate-100 dark:border-zinc-800 opacity-80 group"
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div
                        className={`p-1.5 sm:p-2 rounded-lg sm:rounded-xl ${h.status === 'Completed' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' : 'bg-red-100 dark:bg-red-900/30 text-red-600'}`}
                      >
                        {h.status === 'Completed' ? (
          ...
          <div key={idx} className="relative">
            ...
            <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-4 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                ...
                <button
                  onClick={() =>
                    setCaseData({ ...caseData, facts: caseData.facts.filter((_, i) => i !== idx) })
                  }
        ...
        <button
          onClick={() =>
            setCaseData({
              ...
              facts: [
                ...(caseData.facts || []),
                { event: 'New Event', date: new Date().toISOString(), description: '' },
              ],
            })
          }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div
        className={`p-4 rounded-2xl border flex items-center justify-between ${
          caseData.intelligence?.riskLevel === 'High' ||
          caseData.intelligence?.riskLevel === 'Critical'
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest opacity-80">
              {tLegal('currentlyFlaggedAs')} {caseData.intelligence?.riskLevel || 'Medium'} Risk
            </p>
            {caseData.intelligence?.weakPoints?.[0] && (
              <p className="text-[11px] mt-1 opacity-70 font-medium leading-snug">
                {caseData.intelligence.weakPoints[0]}
              </p>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-red-600 dark:text-red-400">
            ...
            <h4 className="text-xs font-black uppercase tracking-wider">
              {tLegal('criticalVulnerabilities')}
            </h4>
          </div>
          <div className="space-y-2">
            {(caseData.intelligence?.weakPoints || []).map((w, i) => (
              <div
                key={i}
                className="flex gap-3 bg-white dark:bg-[#1A2540] p-4 rounded-xl border border-slate-200 dark:border-white/5 text-sm font-medium text-slate-700 dark:text-slate-300"
              >
                <AlertCircle size={16} className="text-red-500 shrink-0 mt-0.5" />
                {w}
              </div>
            ))}
            {(!caseData.intelligence?.weakPoints ||
              caseData.intelligence?.weakPoints.length === 0) && (
              <div className="text-center py-6 text-subtext italic text-[11px] bg-slate-50 dark:bg-zinc-800/50 rounded-xl border border-dashed border-slate-200 dark:border-zinc-800">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
            <Lightbulb size={18} />
            <h4 className="text-xs font-black uppercase tracking-wider">
              {tLegal('opponentStrategyPrediction')}
            </h4>
          </div>
          <div className="space-y-2">
  ...
  const handleRemovePrecedent = async precedentId => {
    const updatedSaved = (caseData.savedPrecedents || []).filter(
      p => (p._id || p.case_identity?.case_name) !== precedentId
    );
  ...
  const handleUsePrecedentInArgument = caseItem => {
    ...
    const reasoningText = Array.isArray(reasoning)
      ? reasoning[0]
      : reasoning?.slice(0, 150) || "the court's reasoning";
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(caseData.savedPrecedents || []).map((p, i) => {
            ...
            return (
              <div
                key={i}
                className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-[20px] p-5 shadow-sm hover:shadow-md transition-all group flex flex-col h-full relative overflow-hidden"
              >
          ...
          {(!caseData.savedPrecedents || caseData.savedPrecedents.length === 0) && (
            <div className="col-span-full py-12 flex flex-col items-center justify-center bg-slate-50/50 dark:bg-[#131C31] rounded-3xl border-2 border-dashed border-slate-200 dark:border-white/5">
              ...
              <h6 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">
                No precedents saved yet
              </h6>
              <p className="text-[10px] text-subtext mt-1 max-w-[200px] text-center font-medium">
                Use the Legal Precedents tool to find and save relevant case laws.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          {(caseData.research || []).map((r, i) => (
            <div
              key={i}
              className="bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-2xl p-5 shadow-sm group hover:border-amber-200/50 transition-all"
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <div className="px-2.5 py-1 bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 rounded-lg text-[10px] font-black uppercase tracking-wider border border-amber-100/50 dark:border-amber-900/30">
                    {r.lawName || 'Law Reference'}
                  </div>
                  {r.section && (
                ...
                <button
                  onClick={() =>
                    setCaseData({
                      ...caseData,
                      research: caseData.research.filter((_, idx) => idx !== i),
                    })
                  }
                  className="p-2 text-subtext hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-xl transition-all"
                >
                  <X size={14} />
                </button>
              </div>
              <p className="text-xs text-subtext leading-relaxed mb-4 font-medium">
                {r.description}
              </p>
              {r.referenceUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-white/5 rounded-lg text-[10px] font-black text-indigo-500 uppercase tracking-widest hover:bg-indigo-50 transition-all"
                >
          ...
          <button
            onClick={() =>
              setCaseData({
                ...
                research: [
                  ...(caseData.research || []),
                  { lawName: 'New Act', section: '', description: '' },
                ],
              })
            }
            ...
            <div className="relative shrink-0 overflow-hidden">
              ...
              <div className="relative z-10 p-3 sm:p-6 flex items-center justify-between text-white">
                <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
                  ...
                  <div className="overflow-hidden">
                    <h2 className="text-sm sm:text-xl font-black tracking-tight leading-tight uppercase truncate max-w-[160px] sm:max-w-[280px]">
                      {caseData.name}
                    </h2>
                    <div className="flex items-center gap-1 sm:gap-2 mt-0.5 sm:mt-1 overflow-hidden">
                      <span className="text-[7px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-[0.2em] opacity-80 whitespace-nowrap">
                        AI Intelligence
                      </span>
                      <span className="w-1 h-1 bg-white/40 rounded-full shrink-0" />
                      <span className="text-[7px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-[0.2em] opacity-80 truncate">
                        {caseData.caseType || 'Legal'}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <LanguageToggle
                    ...
                    className="mr-1 sm:mr-2 border-white/20 bg-white/10"
                  />
                  <button
                    onClick={e => {
                      e.stopPropagation();
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      onChange={e => {
                        ...
                        setCaseData({
                          ...caseData,
                          evidence: [...(caseData.evidence || []), ...newDocs],
                        });
                  ...
                  <div className="space-y-3">
                    {(caseData.evidence || []).map((doc, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between p-4 bg-white dark:bg-[#1A2540] border border-slate-200 dark:border-white/5 rounded-2xl group"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-black text-slate-800 dark:text-white truncate max-w-[180px]">
                              {doc.name}
                            </p>
                            <p className="text-[9px] font-bold text-subtext uppercase tracking-wider mt-0.5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            onClick={() =>
                              setCaseData({
                                ...caseData,
                                evidence: caseData.evidence.filter((_, idx) => idx !== i),
                              })
                            }
                            className="p-1.5 text-subtext hover:text-red-500 transition-colors"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
                    ...
                    {(!caseData.evidence || caseData.evidence.length === 0) && (
                      <div className="text-center py-12 bg-slate-50/50 dark:bg-zinc-800/10 rounded-2xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-subtext">
                          Vault is Empty
                        </p>
                        <p className="text-[9px] text-subtext/60 mt-1">
                          Upload relevant documents, proofs, or notices.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  ...
                  <div className="space-y-3">
                    {(caseData.tasks || []).map((task, i) => (
                      <div
                        key={i}
                        className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-4 flex items-center justify-between group"
                      >
                        <div className="flex items-center gap-4 flex-1">
                          <button
                            onClick={() => {
                              ...
                              newTasks[i].status =
                                newTasks[i].status === 'Completed' ? 'Pending' : 'Completed';
                        ...
                        <div className="flex items-center gap-3">
                          <span
                            className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest ${
                              task.priority === 'High'
                                ...
                                : 'bg-slate-50 text-subtext border border-slate-100'
                            }`}
                          >
                          ...
                          <button
                            onClick={() =>
                              setCaseData({
                                ...caseData,
                                tasks: caseData.tasks.filter((_, idx) => idx !== i),
                              })
                            }
                    ...
                    {(!caseData.tasks || caseData.tasks.length === 0) && (
                      <div className="text-center py-12 bg-slate-50/50 dark:bg-zinc-800/10 rounded-2xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
                        ...
                        <p className="text-[9px] text-subtext/60 mt-1">
                          Auto-Analyze to generate a legal timeline.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              {selectedPrecedent && (
                <CaseDetailView
                  ...
                  onCopyCitation={() => {
                    ...
                    const citation =
                      p.case_identity?.citation || p.citation || 'Citation unavailable';
                    let textToCopy = `${name}`;
