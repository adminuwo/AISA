...
const FullScreenCaseAssistant = ({
  ...
  const handleExportChatLog = () => {
    ...
    const logText = aiMessages
      .map(m => `[${m.role === 'user' ? 'ADVOCATE' : 'AI COPILOT'}] ${m.content}\n`)
      .join('\n');
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-[#FAFBFD] flex flex-col h-full overflow-hidden select-text text-slate-800">
          ...
          <div className="flex items-center gap-1">
            ...
            <div>
              <h1 className="text-sm font-black text-slate-900 uppercase tracking-wider leading-tight">
                {caseData
                  ? caseData.title || caseData.name || caseData.caseTitle || 'Case Assistant'
                  : 'Case Assistant'}
              </h1>
              {caseData && (
                <p className="text-[9px] font-bold text-[#4F46E5] uppercase tracking-widest leading-none mt-0.5">
                  {[
                    caseData.courtName || caseData.court,
                    caseData.caseType || caseData.category,
                    caseData.stage,
                  ]
                    .filter(Boolean)
                    .join(' · ') || 'Case Workspace Assistant'}
                </p>
              )}
            </div>
          </div>
                  ...
                  {[
                    {
                      ...
                      act: () => {
                        exportToPDF({
                          ...
                          text: aiMessages.map(m => m.content).join('\n\n'),
                    ...
                    },
                    {
                      ...
                      act: () => {
                        const blob = new Blob([aiMessages.map(m => m.content).join('\n\n')], {
      ...
      <main
        ...
        className="flex-1 overflow-y-auto px-4 py-8 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent relative"
      >
              ...
              >
                ...
                <div className="p-6 border border-[#4F46E5]/15 bg-white rounded-3xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                  <div className="space-y-4 flex-1">
                    <div>
                      <span className="text-[9px] font-black uppercase text-[#4F46E5] tracking-widest block mb-1">
                        AI LEGAL COPILOT READY
                      </span>
                      ...
                      <p className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mt-1">
                        {caseData?.courtName || 'District Court Jabalpur'} •{' '}
                        {caseData?.caseType || 'Civil Property Dispute'}
                      </p>
                    </div>
                    ...
                    <div className="flex justify-end items-start gap-3 pl-16">
                      <div className="flex flex-col items-end gap-1">
                        ...
                        <div className="flex items-center gap-2 text-[9px] text-slate-400 font-bold uppercase tracking-wider px-1">
                          <span>
                            {new Date(Number(msg.id) || Date.now()).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          </span>
                      ...
                      <div className="bg-white border border-slate-100 rounded-[18px] p-6 sm:p-8 shadow-sm leading-relaxed text-slate-800 text-[13px] hover:shadow-md transition-shadow">
                        ...
                        {!msg.isGenerating && (
                          <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 font-bold shrink-0">
                            <span className="text-[9px] font-black uppercase tracking-widest text-[#4F46E5] bg-[#4F46E5]/10 px-2 py-0.5 rounded">
                              Ready
                            </span>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="relative">
                                <button
                                  type="button"
                                  onClick={() =>
                                    setActiveMoreMenuMsgId(
                                      activeMoreMenuMsgId === msg.id ? null : msg.id
                                    )
                                  }
                                      ...
                                      {[
                                        {
                                          ...
                                          act: () => {
                                            window.__aisa_fullscreen_send_message?.(
                                              'Explain in detail the previous response'
                                            );
                                          },
                                        },
                                        {
                                          ...
                                          act: () => {
                                            window.__aisa_fullscreen_send_message?.(
                                              'Simplify to plain English the previous response'
                                            );
                                          },
                                        },
                                        {
                                          ...
                                          act: () => {
                                            window.__aisa_fullscreen_send_message?.(
                                              'Expand with more legal grounds the previous response'
                                            );
                                          },
                                        },
                                        {
                                          ...
                                          act: () => {
                                            window.__aisa_fullscreen_send_message?.(
                                              'Translate the previous response to Hindi'
                                            );
                                          },
                                        },
                                        {
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-4 custom-scrollbar text-xs">
              ...
              {['Pinned Chats', "Today's Chats", 'Yesterday', 'Older Chats'].map(groupName => {
                ...
                if (groupName === 'Pinned Chats') {
                  ...
                  groupSessions = filtered.filter(
                    s => !pinnedSessions.includes(s.chat_id) && nowTime - s.timestamp >= 2 * oneDay
                  );
                }
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar text-xs">
              ...
              <div className="space-y-4">
                <div className="p-4 border border-slate-100 bg-slate-50/50 rounded-2xl space-y-3">
                  ...
                  <h5 className="text-sm font-black text-slate-850 uppercase tracking-tight leading-none">
                    {caseData?.title || caseData?.name || 'Rajesh Sharma vs Amit Verma'}
                  </h5>
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div className="p-3 border border-slate-100 bg-white rounded-xl">
                    ...
                    <span className="text-xs font-bold text-slate-700 block mt-1">
                      {caseData?.clientName || 'Rajesh Sharma'}
                    </span>
                  </div>
                  <div className="p-3 border border-slate-100 bg-white rounded-xl">
                    <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">
                    ...
                    <span className="text-xs font-bold text-slate-700 block mt-1">
                      {caseData?.opponentName || 'Amit Verma'}
                    </span>
                  </div>
                  ...
                  <div className="p-3 border border-slate-100 bg-white rounded-xl">
                    ...
                    <span className="text-xs font-bold text-[#4F46E5] block mt-1">
                      {caseData?.nextHearingDate || '15 Jul 2026'}
                    </span>
                  </div>
                  <div className="p-3 border border-slate-100 bg-white rounded-xl">
                    <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider">
                    ...
                    <span
                      className="text-xs font-bold text-[#EF4444] block mt-1 truncate"
                      title={caseData?.deadlines || 'CPC Order 37 Reply'}
                    >
                      {caseData?.deadlines || '12 Jul 2026'}
                    </span>
                  </div>
                  ...
                  <div className="p-3 border border-slate-100 bg-white rounded-xl flex items-center justify-between col-span-2">
                    <div>
                      ...
                      <span className="text-xs font-bold text-slate-750 block mt-0.5">
                        {caseData?.tasks?.filter(t => t.status !== 'Done').length || 3} items
                      </span>
                    </div>
                    <div className="flex gap-1.5 text-[9px] font-black text-slate-400">
                      <span className="px-2 py-0.5 bg-slate-100 rounded">
                        Evidence: {caseData?.documents?.length || 4}
                      </span>
                      <span className="px-2 py-0.5 bg-slate-100 rounded">
                        Drafts: {caseData?.drafts?.length || 2}
                      </span>
                      <span className="px-2 py-0.5 bg-slate-100 rounded">
                        Research: {caseData?.research?.length || 3}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-4 border border-[#4F46E5]/10 bg-indigo-50/30 rounded-2xl space-y-2">
                  ...
                  <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${caseData?.probability || 60}%` }}
                    />
                  </div>
                </div>
