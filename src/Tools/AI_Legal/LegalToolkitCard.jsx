...
const LegalToolkitCard = ({ isOpen, onClose, onSelect, unlockedTools = [], isAdmin = false }) => {
  ...
  const SectionTitle = ({ children }) => (
    <div className="flex items-center gap-4 mb-5 mt-8 first:mt-0">
      ...
      <h4 className="text-[8px] sm:text-[9px] font-black text-slate-500 dark:text-[#94A3B8] uppercase tracking-[0.28em] whitespace-nowrap">
        {children}
      </h4>
  ...
  return createPortal(
    <AnimatePresence mode="wait">
      {isOpen && (
        ...
        >
          {/* Backdrop */}
          <div onClick={onClose} className="absolute inset-0 bg-black/60 backdrop-blur-[12px]" />

          {/* Outer wrapper */}
          <div
            className={`relative z-10 w-full flex items-center justify-center ${isMaximized ? 'h-full' : 'max-w-5xl'}`}
          >
            {/* Main card */}
            <motion.div
              ...
              style={{
                boxShadow:
                  '0 40px 80px -15px rgba(0,0,0,0.4), inset 0 2px 4px rgba(255,255,255,0.1)',
                maxHeight: '90vh',
              }}
            >
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3.5">
                  ...
                  <div>
                    <h1 className="text-[17px] font-black text-slate-900 dark:text-white leading-tight tracking-tight">
                      {tLegal('legalToolkitTitle')}
                    </h1>
                    <div className="flex items-center gap-1">
                      <Sparkles className="w-2.5 h-2.5 text-indigo-500 animate-pulse" />
                      <span className="text-[7.5px] sm:text-[9px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.22em]">
                        {tLegal('legalProfessionalToolkit')}
                      </span>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-3.5 sm:gap-6 w-full sm:w-auto">
                      ...
                      <div className="flex-1 text-left space-y-0.5 sm:space-y-1.5">
                        <div className="flex items-center justify-start gap-1.5 sm:gap-2">
                          <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 bg-white/15 backdrop-blur-md border border-white/20 rounded-full text-[7px] sm:text-[8px] font-black text-white uppercase tracking-widest">
                            {tLegal('basicStatus')}
                          </span>
                          <Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-yellow-400 fill-yellow-400" />
                        </div>
                        <h2 className="text-[17px] leading-tight sm:text-2xl font-extrabold text-white tracking-tight">
                ...
                {!isAdmin && (
                  ...
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h6 className="font-extrabold text-slate-900 dark:text-white text-[15px] sm:text-[17px]">
                          {tLegal('fullLegalSuiteAccess')}
                        </h6>
                        <p className="text-[11px] sm:text-[12px] text-slate-500 dark:text-slate-400 font-semibold">
                          {tLegal('unlockAdvancedTools')}
                        </p>
                      </div>
                    </div>
                    <motion.button
