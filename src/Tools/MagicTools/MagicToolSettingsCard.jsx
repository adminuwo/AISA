...
const ActiveFill = () => <div className="absolute inset-0 bg-primary z-0 pointer-events-none" />;
...
const MagicToolSettingsCard = ({
  ...
  const pricingKey =
    toolType === 'edit'
      ? 'image'
      : ['deepsearch', 'websearch', 'coding', 'chat'].includes(toolType)
        ? 'chat'
        : toolType;

  const toolPricingBase = pricing[pricingKey] || { models: [], editModels: [] };
  const toolPricing = {
    models: toolType === 'edit' ? toolPricingBase.editModels || [] : toolPricingBase.models || [],
  };
              ...
              <div className="relative z-20 px-5 sm:px-6 pt-6 pb-4 border-b border-black/[0.04] bg-white/40">
                ...
                <div className="flex items-center justify-between relative">
                  <div className="flex items-center gap-3.5">
                    <div className="w-[34px] sm:w-[38px] h-[34px] sm:h-[38px] relative z-10 rounded-[10px] sm:rounded-xl bg-gradient-to-br from-primary via-[#4F46E5] to-[#3B82F6] flex items-center justify-center shadow-lg border border-white/30">
                  ...
                  <div className="flex items-center gap-2 relative z-10">
                    ...
                    <motion.button
                      whileHover={{
                        scale: 1.1,
                        backgroundColor: isDark ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.7)',
                        rotate: 90,
                      }}
                      whileTap={{ scale: 0.9 }}
                      onClick={onClose}
                  ...
                  <div>
                    {toolType === 'video' && (
                      <div className="flex items-center gap-2 mb-3 ml-1">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-[0.25em] text-slate-800/80 dark:text-white/80 drop-shadow-sm">
                          Aspect Ratio
                        </p>
                      </div>
                    )}
                    ...
                    <div
                      className={`relative grid ${toolType === 'video' ? 'grid-cols-3' : 'grid-cols-4'} gap-1.5 bg-white/50 dark:bg-white/5 p-1.5 rounded-[16px] border border-white/60 dark:border-white/10 shadow-[inset_0_2px_12px_rgba(0,0,0,0.03)] backdrop-blur-xl`}
                    >
                        ...
                        {aspectRatios.map(ar => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                className={`relative z-10 flex flex-col items-center transition-all duration-300 ${isActive ? 'scale-105' : 'scale-100 opacity-70 group-hover:opacity-100'}`}
                              >
                                ...
                                <span
                                  className={`text-[9.5px] font-black tracking-tighter ${isActive ? 'text-white' : 'text-slate-600 dark:text-zinc-400 group-hover:text-slate-900 dark:group-hover:text-white'}`}
                                >
                                  {ar.label}
                                </span>
                              </div>
                                ...
                                <div className="flex items-center gap-3.5 relative z-10 w-full">
                                  ...
                                  >
                                    {model.speed === 'Fast' ? (
                                      ...
                                      <Zap size={18} className={isActive ? 'drop-shadow-md' : ''} />
                                    )}
                                  </motion.div>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between mb-1">
                                      <span
                                        className={`text-[14px] font-black truncate pr-2 transition-colors ${isActive ? (isDark ? 'text-white' : 'text-slate-900') : isDark ? 'text-zinc-300' : 'text-slate-700'}`}
                                      >
                                        {model.name}
                                      </span>
                                    ...
                                    <p
                                      className={`text-[10.5px] font-medium leading-snug transition-colors line-clamp-2 ${isActive ? (isDark ? 'text-zinc-400' : 'text-slate-500') : isDark ? 'text-zinc-500' : 'text-slate-400 group-hover:text-slate-600'}`}
                                    >
                                      {model.description}
                                    </p>
                                  </div>
              ...
              <div className="p-4 relative z-20 border-t border-black/[0.04] bg-white/40 dark:bg-zinc-800/40 backdrop-blur-md">
                ...
                >
                  <div className="relative z-10 flex items-center justify-center gap-1.5">
                    ...
                    <span className="text-[11.5px] font-black uppercase tracking-[0.2em]">
                      Activate Core
                    </span>
                  </div>
                </button>
