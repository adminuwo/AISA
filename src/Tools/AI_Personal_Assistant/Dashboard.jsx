...
const Dashboard = () => {
  ...
  const triggerNotification = task => {
    ...
    toast.custom(
      toastObj => (
        <div
          className={`${toastObj.visible ? 'animate-enter' : 'animate-leave'} max-w-md w-full bg-white dark:bg-[#1E1E1E] shadow-2xl rounded-2xl pointer-events-auto flex ring-1 ring-black ring-opacity-5 border border-primary/20 overflow-hidden`}
        >
          <div className="flex-1 w-0 p-4">
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1 min-w-0">
                ...
                {task.isUrgent && (
                  <span className="mt-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-red-500 text-white shadow-sm">
                    {t('urgent')}
                  </span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-white dark:bg-[#1A1A1A] backdrop-blur-xl p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 border-t-4 border-t-blue-500 relative overflow-hidden group hover:shadow-lg transition-all">
            ...
            <h3 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mt-2">
              {
                tasks.filter(t => new Date(t.datetime).toDateString() === new Date().toDateString())
                  .length
              }
            </h3>
          </div>
          ...
          <div className="bg-white dark:bg-[#1A1A1A] backdrop-blur-xl p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 border-t-4 border-t-primary relative overflow-hidden group hover:shadow-lg transition-all">
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-primary mt-1">{tasks.length}</h3>
          </div>
        </div>
              ...
              >
                ...
                <div className="grid gap-4">
                  {items.map(task => (
                    <div
                      key={task._id}
                      className={`group bg-white dark:bg-[#1A1A1A] backdrop-blur-sm p-3 md:p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col md:flex-row items-start md:items-center gap-3 transition-all hover:shadow-md hover:scale-[1.002] hover:border-primary/30 ${task.isUrgent ? 'border-l-4 border-l-red-500 bg-red-50/30 dark:bg-red-900/10' : ''}`}
                    >
                      <button
                        onClick={() => toggleComplete(task)}
                        className={`mt-1 md:mt-0 flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h3
                            className={`text-lg font-semibold truncate ${task.status === 'completed' ? 'text-gray-400 line-through' : 'text-gray-900 dark:text-gray-100'}`}
                          >
                            {task.title}
                          </h3>
                          {task.isUrgent && <AlertTriangle className="w-4 h-4 text-red-500" />}
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${getStatusColor(task.status)}`}
                          >
                            {t(task.status)}
                          </span>
                        </div>
