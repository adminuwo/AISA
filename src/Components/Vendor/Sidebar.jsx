...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed lg:sticky top-0 left-0 h-screen w-68 bg-card border-r border-border flex flex-col z-40 transition-all duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Brand Logo Section */}
        <div className="h-16 flex items-center px-8 border-b border-border/50">
          <div className="flex items-center space-x-3">
            {/* Logo Removed as per request */}
            <span className="text-xl font-black text-maintext tracking-tighter uppercase">
              Vendor
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-8 px-4 space-y-2">
          <div className="px-4 mb-4">
            <p className="text-[10px] font-black text-subtext uppercase tracking-[0.2em]">
              Management
            </p>
          </div>
          {menuItems.map(item => {
            ...
            return (
              <div key={item.name}>
                ...
                >
                  <item.icon
                    size={20}
                    className={`mr-3 transition-colors ${active || groupActive ? 'text-white' : 'text-subtext group-hover:text-maintext'}`}
                  />
                  {item.name}
                </Link>
