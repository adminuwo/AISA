...
const LiveAI = ({ onClose, language }) => {
  ...
  const resumeTypewriter = () => {
    const type = () => {
      ...
      if (typewriterIndexRef.current < typewriterTextRef.current.length) {
        setDisplayedText(
          prev => prev + typewriterTextRef.current.charAt(typewriterIndexRef.current)
        );
  ...
  const startTypewriter = (text, duration = null) => {
    ...
    console.log(
      `⌨️ [LiveAI] Typewriter speed: ${speed.toFixed(2)}ms/char for duration: ${duration}s`
    );
  ...
  const speakResponse = async text => {
    ...
    const targetLang = /[\u0900-\u097F]/.test(text)
      ? 'hi-IN'
      : language === 'Hindi'
        ? 'hi-IN'
        : langMap[language] || 'en-US';
    ...
    try {
      ...
      console.log(
        `🎤 [LiveAI] Requesting Backend TTS: Lang=${targetLang}, Gender=${voiceGenderRef.current}`
      );
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 md:p-8 flex justify-center items-center bg-gradient-to-t from-black/80 via-black/40 to-transparent">
        <div className="flex items-center justify-evenly gap-1 xs:gap-2 sm:gap-4 md:gap-6 bg-zinc-900/50 backdrop-blur-xl border border-white/10 px-2 xs:px-6 md:px-8 py-2 sm:py-4 rounded-full shadow-2xl max-w-[98vw] flex-nowrap overflow-x-auto no-scrollbar">
          ...
          <button
            ...
            className="p-1.5 xs:p-2 sm:p-3 rounded-full text-white/80 hover:text-white hover:bg-white/10 transition-all shrink-0"
            title="Switch Camera"
          >
          ...
          <button
            ...
            className="p-1.5 xs:p-2 sm:p-3 rounded-full text-white/80 hover:text-white hover:bg-white/10 transition-all font-bold text-base xs:text-lg sm:text-xl shrink-0"
          ...
          {(isSpeaking || isPaused) && (
            ...
            >
              {isPaused ? (
                <Play className="w-4.5 h-4.5 xs:w-5 xs:h-5 sm:w-6 sm:h-6 fill-current" />
              ) : (
                <Pause className="w-4.5 h-4.5 xs:w-5 xs:h-5 sm:w-6 sm:h-6 fill-current" />
              )}
            </button>
          )}
          ...
          <button
            onClick={toggleVideo}
            className={`p-1.5 xs:p-2 sm:p-3 rounded-full transition-all shrink-0 ${isVideoActive ? 'text-white/80 hover:text-white hover:bg-white/10' : 'text-red-400 bg-red-500/10'}`}
            title={isVideoActive ? 'Turn Off Video' : 'Turn On Video'}
          >
            {isVideoActive ? (
              <Video className="w-4.5 h-4.5 xs:w-5 xs:h-5 sm:w-6 sm:h-6" />
            ) : (
              <VideoOff className="w-4.5 h-4.5 xs:w-5 xs:h-5 sm:w-6 sm:h-6" />
            )}
          </button>
          ...
          <button
            ...
            className={`h-11 w-11 xs:h-12 xs:w-12 md:h-16 md:w-16 rounded-full flex items-center justify-center transition-all duration-500 shadow-lg shrink-0 ${isListening ? 'bg-white text-black scale-105 xs:scale-110 shadow-blue-500/50' : 'bg-red-500 text-white hover:bg-red-600 hover:scale-105'}`}
          ...
          <button
            ...
            className="p-1.5 xs:p-2 sm:p-3 rounded-full bg-zinc-800 text-red-500 hover:bg-zinc-700 transition-all shrink-0"
            title="End Call"
          >
