...
const MultiScheduleReminder = () => {
  const {
    ...
    reminders,
    setReminders,
    fetchReminders: globalFetchReminders,
  } = usePersonalization();
    ...
    <div className="py-6 border-b border-gray-100 dark:border-white/5 space-y-6">
      ...
      <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/5 rounded-xl border border-border">
        <div className="flex items-center gap-3">
          ...
          >
            <div
              className={`w-4 h-4 rounded-full transition-transform duration-300 shadow-sm bg-white ${isGlobalEnabled ? 'translate-x-5' : 'translate-x-0'}`}
            />
          </button>
          <div>
            <p className="text-sm font-bold">
              {isGlobalEnabled ? 'Reminders Active' : 'Reminders Paused'}
            </p>
            <p className="text-[10px] text-subtext">
              {isGlobalEnabled
                ? 'Your schedules will trigger as expected'
      ...
      {nextReminder && isGlobalEnabled && (
        <motion.div
          ...
          className="flex items-center gap-4 p-3 bg-gradient-to-r from-primary/10 to-transparent border-l-4 border-primary rounded-r-xl"
        >
          ...
          <div>
            ...
            <p className="text-[10px] text-subtext">
              {new Date(nextReminder.datetime).toLocaleString()}
            </p>
          </div>
        </motion.div>
      )}
      ...
      {/* Reminders List */}
      <div
        className={`space-y-3 transition-opacity duration-300 ${!isGlobalEnabled ? 'opacity-50 pointer-events-none' : ''}`}
      >
        {loading ? (
          ...
          <div className="py-10 text-center flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-xs text-subtext max-w-[200px] mx-auto">
              Click 'Add Reminder' to get started and manage your schedule.
            </p>
          </div>
        ) : (
            ...
            <div key={reminder._id} className="relative overflow-hidden group">
              ...
              <div className="relative z-10 p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {reminder.status === 'completed' && (
                      <span className="px-2 py-0.5 bg-green-500/20 text-green-500 text-[9px] font-black rounded-full uppercase tracking-wider">
                        Finished
                      </span>
                    )}
                  </div>
                  {reminder.description && (
                    <p className="text-xs text-subtext truncate mb-2">{reminder.description}</p>
                  )}
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  ...
                  >
                    <div
                      className={`w-4 h-4 rounded-full transition-transform duration-300 shadow-sm bg-white ${reminder.isActive ? 'translate-x-4' : 'translate-x-0'}`}
                    />
                  </button>

                  <button
                    onClick={() => openModal(reminder)}
                    className="p-2 text-gray-400 hover:text-primary transition-colors hover:bg-gray-100 dark:hover:bg-white/5 rounded-lg border border-transparent"
                  >
                    <Edit2 size={14} />
                  </button>
          ...
          <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full mt-1 bg-gray-50 dark:bg-black/20 border border-border rounded-xl p-3 text-sm outline-none focus:border-primary transition-all resize-none h-20"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full mt-1 bg-gray-50 dark:bg-black/20 border border-border rounded-xl p-3 text-sm outline-none focus:border-primary transition-all"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <div className="relative mt-1">
                    <select
                      ...
                      className="w-full bg-gray-50 dark:bg-black/20 border border-border rounded-xl p-3 text-sm outline-none focus:border-primary transition-all appearance-none pr-10"
                    >
                ...
                <div>
                  ...
                  <p className="text-[9px] text-subtext mt-1">
                    Select a voice to hear your reminder aloud when it triggers.
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-black/20 flex justify-end gap-3">
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="px-5 py-2.5 rounded-xl font-bold text-sm text-gray-500 hover:bg-gray-200 dark:hover:bg-white/5 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={saveReminder}
                  className="px-5 py-2.5 rounded-xl font-bold text-sm text-white bg-primary shadow-lg shadow-primary/20 hover:opacity-90 transition-opacity flex items-center gap-2"
                >
