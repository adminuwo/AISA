...
const ProfileSettingsDropdown = ({ onClose, onLogout }) => {
  ...
  const renderSettingRow = (label, description, control) => (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-5 border-b border-gray-100 dark:border-white/5 last:border-0 gap-4">
      <div className="flex flex-col gap-1 pr-4 flex-1">
        <span className="text-sm font-black text-gray-800 dark:text-gray-100 tracking-tight">
          {label}
        </span>
        {description && (
          <span className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed font-medium">
            {description}
          </span>
        )}
      </div>
  ...
  const renderToggle = (value, onToggle) => (
    ...
    >
      <div
        className={`w-4 h-4 rounded-full transition-transform duration-300 shadow-sm bg-white ${value ? 'translate-x-5' : 'translate-x-0'}`}
      />
    </button>
  );
  ...
  const allSettings = useMemo(() => {
    ...
    settings.push({
      ...
      component: renderSettingRow(
        ...
        renderDropdown(
          ...
          e =>
            setTheme(
              e.target.value === t('system')
                ? 'system'
                : e.target.value === t('dark')
                  ? 'dark'
                  : 'light'
            ),
          Monitor
      ...
      component: renderSettingRow(
        ...
        <div className="flex items-center gap-3">
          <div
            ...
            style={{
              backgroundColor: `hsl(${ACCENT_COLORS[accentColor] || ACCENT_COLORS['Default']})`,
            }}
          />
          {renderDropdown(
            accentColor,
    ...
    settings.push({
      ...
      keywords: 'country region location',
      component: renderSettingRow(
        t('region'),
        t('regionDesc'),
        renderDropdown(region, Object.keys(regions), e => setRegion(e.target.value), Globe)
      ),
    });
    settings.push({
      id: 'language',
      ...
      keywords: 'language translation hindi english',
      component: renderSettingRow(
        t('language'),
        t('languageDesc'),
        renderDropdown(language, regions[region] || [], e => setLanguage(e.target.value), Languages)
      ),
    });

    // Personalization
    settings.push({
      id: 'multiScheduleReminder',
      tab: 'personalization',
      label: 'Multi Schedule Reminder',
      keywords: 'alarm scheduler calendar schedule',
    ...
    settings.push({
      ...
      keywords: 'save toggle',
      component: renderSettingRow(
        t('chatHistory'),
        t('chatHistoryDesc'),
        renderToggle(personalizations.dataControls?.chatHistory === 'On', val =>
          updatePersonalization('dataControls', { chatHistory: val ? 'On' : 'Off' })
        )
      ),
    });
        ...
        <div className="flex flex-col gap-2 py-4 border-b border-gray-100 dark:border-white/5">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-gray-50 dark:bg-zinc-800 border rounded-xl p-3 text-sm outline-none focus:border-primary transition-all"
            />
            <button
              onClick={handleSaveNickname}
              className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 bg-primary text-white text-[10px] rounded-lg hover:opacity-90 transition-all font-bold"
            >
              {t('saveLabel')}
            </button>
          </div>
    ...
    if (searchQuery.trim()) {
      ...
      return (
        <div className="space-y-4">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-4 px-1">
            {t('searchResults') || 'Search Results'}
          </h3>
          {results.map(item => (
            <div
              key={item.id}
              className="bg-gray-50/50 dark:bg-white/[0.02] rounded-2xl p-4 border border-border/50"
            >
              {item.component}
            </div>
          ))}
      ...
      case 'personalization':
        return (
          <div className="space-y-0">
            {allSettings
              .filter(s => s.tab === 'personalization')
              .map(s => (
            ...
            <div className="space-y-3">
              {notifications.length > 0 ? (
                notifications.map(n => (
                  <div
                    key={n.id}
                    className="p-4 bg-gray-50/80 dark:bg-white/5 rounded-2xl border border-border/50 flex items-start justify-between gap-4 transition-all hover:border-primary/20"
                  >
        ...
        return (
          <div className="space-y-6">
            <div className="bg-primary/5 rounded-[2.5rem] p-5 sm:p-8 border border-primary/20 shadow-xl shadow-primary/5">
              ...
              <p className="text-xs sm:text-sm text-subtext mb-8 leading-relaxed font-medium">
                Your feedback helps us build the future of AISA™. Tell us what you'd like to see
                next.
              </p>
              <textarea
                rows={5}
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <div className="p-5 sm:p-6 bg-gradient-to-br from-primary/10 to-transparent rounded-[2rem] sm:rounded-3xl border border-primary/10">
              <div className="flex items-center gap-3 sm:gap-4 mb-3 sm:mb-4">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-maintext tracking-tight">
                    {isPrivacy ? t('privacyPolicy') : t('termsOfService')}
                  </h2>
                  <p className="text-[10px] text-subtext font-bold uppercase tracking-widest mt-0.5 sm:mt-1 opacity-60">
                    {t('lastUpdated')}: Jan 2026
                  </p>
                </div>
              </div>
              <p className="text-[11px] sm:text-xs text-subtext leading-relaxed font-medium">
                Please read our {isPrivacy ? 'privacy policy' : 'terms of service'} carefully to
                understand how we handle your data and the rules of our platform.
              </p>
            </div>
                ...
                {Object.keys(groupedSessions).length > 0 ? (
                  Object.keys(groupedSessions)
                    ...
                    .map(date => (
                      <div
                        key={date}
                        className="border border-border rounded-xl bg-gray-50/50 dark:bg-zinc-800/30"
                      >
                        <button
                        ...
                        >
                          ...
                          <ChevronDown
                            size={14}
                            className={`transition-transform ${expandedDate === date ? 'rotate-180' : ''}`}
                          />
                        </button>
                        {expandedDate === date && (
                          <div className="p-2 pt-0 space-y-1">
                            {groupedSessions[date].map(s => (
                              <div
                                key={s.sessionId}
                                className="flex items-center justify-between p-2 hover:bg-white dark:hover:bg-zinc-800 rounded-lg text-xs group"
                              >
                                <span className="truncate flex-1 font-medium text-gray-600 dark:text-gray-300">
                                  {s.title || t('newChat')}
                                </span>
                                <div className="flex items-center gap-1">
                                  <button
                                    onClick={() => {
                                      window.location.href = `/dashboard/chat/${s.sessionId}`;
                                      onClose();
                                    }}
                                  ...
                                  <button
                                    onClick={e => {
                                      e.stopPropagation();
                                      handleDeleteChatSession(s.sessionId);
                                    }}
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-6 text-center">
              ...
              <div>
                ...
                <p className="text-sm text-subtext max-w-xs">
                  Please log in to access your account settings, sessions, and profile information.
                </p>
              </div>
              <button
                onClick={() => {
          ...
          <div className="space-y-6">
                ...
                >
                  ...
                  <div className="w-14 h-14 sm:w-24 sm:h-24 rounded-2xl bg-white dark:bg-zinc-900 flex items-center justify-center text-primary border border-primary/20 shadow-lg overflow-hidden relative z-10 transition-transform group-hover/avatar:scale-105">
                    {user.avatar ? (
                      ...
                      <span className="text-3xl font-black group-hover/avatar:opacity-50 transition-opacity">
                ...
                <div className="flex-1 text-center sm:text-left space-y-1.5 sm:space-y-3">
                  <div>
                    <h3 className="text-xl sm:text-2xl font-black text-gray-800 dark:text-white capitalize flex items-center justify-center sm:justify-start gap-2 sm:gap-3">
                      {user.name}
                      <span
                        className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-widest border shadow-sm ${
                    ...
                    <p className="text-xs sm:text-sm font-bold text-gray-500/80 mt-0.5 sm:mt-1">
                      {user.email}
                    </p>
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-subtext font-bold uppercase tracking-wider">
                ...
                {Array.isArray(sessions) && sessions.length > 0 ? (
                  sessions.map(session => (
                    <motion.div
                      ...
                      key={session._id}
                      className={`group relative p-4 sm:p-5 rounded-3xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
                        session.isCurrent
                  ...
                  <div className="py-12 text-center border-2 border-dashed border-border/40 rounded-3xl bg-gray-50/50 dark:bg-white/[0.01]">
                    ...
                    <p className="text-xs font-bold text-subtext/60 uppercase tracking-widest">
            ...
            <div className="py-4 flex justify-between items-center text-sm border-b border-gray-100 dark:border-white/5">
              <div>
                ...
                <p className="text-xs text-subtext">
                  {t('manageAccountSecurity') || 'Manage your account security'}
                </p>
              </div>
              <button
            ...
            <div className="py-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-subtext leading-tight max-w-[280px]">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-2 mb-4">
                  <h2 className="text-3xl font-black text-maintext">
                    {planName.replace(' Plan', '')}
                  </h2>
                  {renewalDate && (
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-3 bg-primary text-white rounded-xl text-xs font-bold shadow-lg shadow-primary/20 hover:opacity-90 transition-all uppercase tracking-widest"
              >
                Upgrade Plan
              </button>
            </div>
                ...
                <div className="space-y-3 mt-6">
                  {faqs[selectedFaqCategory].questions
                    ...
                    .map((faq, index) => (
                      <div
                        key={index}
                        className="bg-gray-50/50 dark:bg-white/5 rounded-2xl border border-transparent hover:border-primary/10 transition-all"
                      >
                        <button
                          onClick={() => setOpenFaqIndex(openFaqIndex === index ? null : index)}
                          className="w-full flex justify-between items-center p-5 text-left group"
                        >
                          <span className="font-bold text-gray-700 dark:text-gray-200 text-sm group-hover:text-primary transition-colors">
                            {faq.question}
                          </span>
                          <div
                            className={`p-1.5 rounded-lg transition-all ${openFaqIndex === index ? 'bg-primary/10 text-primary rotate-180' : 'text-gray-400'}`}
                          >
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <ChevronDown
                          size={16}
                          className={`transition-transform ${isIssueDropdownOpen ? 'rotate-180' : ''}`}
                        />
                      </button>
                      <AnimatePresence>
                        {isIssueDropdownOpen && (
      ...
      case 'connectors':
        ...
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div
              className={`relative overflow-hidden rounded-2xl border transition-all duration-300 ${gmailApp ? 'border-primary/30 bg-gradient-to-br from-primary/5 via-transparent to-transparent dark:from-primary/10' : 'border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 hover:border-primary/30'}`}
            >
              {/* Top accent line */}
              {gmailApp && (
              ...
              <div className="p-5">
                ...
                {!gmailApp && (
                  <p className="mt-3 text-[10px] text-gray-400 dark:text-gray-500">
                    💡 Once connected, ask AISA™ to check your emails, search your inbox, send
                    replies, and more — right from chat.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-dashed border-primary/20 dark:border-primary/10 p-5 text-center opacity-60">
              <p className="text-xs font-bold text-primary/60 uppercase tracking-widest">
                {t('moreConnectorsComingSoon')}
              </p>
              <p className="text-[10px] text-gray-400 mt-1">
                Google Drive · Notion · Slack · Calendar
              </p>
            </div>
          </div>
        );
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[9999] flex items-center justify-center pointer-events-none p-0 sm:p-4">
        ...
        >
          ...
          <div
            className={`flex flex-col h-full w-full sm:w-[320px] bg-gray-50/30 dark:bg-black/20 border-r border-gray-100 dark:border-white/5 shrink-0 transition-all ${view === 'detail' ? 'hidden sm:flex' : 'flex'}`}
          >
            {/* Sticky Header */}
            <div className="sticky top-0 z-20 px-6 py-6 flex justify-between items-center bg-transparent shrink-0">
              ...
              <button
                onClick={onClose}
                className="sm:hidden p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-all"
              >
            ...
            <nav className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto custom-scrollbar">
              {tabs.map(tab => (
                ...
                >
                  ...
                  <div
                    className={`p-2 rounded-xl transition-all ${activeTab === tab.id ? 'bg-primary text-white' : 'bg-gray-200/50 dark:bg-white/5 text-gray-400 group-hover:text-primary group-hover:bg-primary/5'}`}
                  >
                    <tab.icon className="w-4 h-4" />
                  </div>
                  <span className="font-black flex-1 text-left">{tab.label}</span>
                  <ChevronRight
                    className={`w-4 h-4 opacity-30 group-hover:opacity-100 group-hover:translate-x-1 transition-all ${activeTab === tab.id ? 'opacity-100 text-primary' : ''}`}
                  />
                </button>
              ))}
            </nav>
            ...
            {user?.token && (
              <div className="p-6 border-t border-gray-100 dark:border-white/5">
                <button
                  onClick={onLogout}
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-[10px] text-red-500 hover:bg-red-500/10 font-black uppercase tracking-[0.2em] transition-all border border-red-500/10 active:scale-95 bg-white/50 dark:bg-transparent shadow-sm"
                >
          ...
          <div
            className={`flex-1 flex flex-col min-w-0 bg-white dark:bg-transparent overflow-hidden ${view === 'sidebar' ? 'hidden sm:flex' : 'flex'}`}
          >
            ...
            <div className="sticky top-0 z-20 px-5 sm:px-8 py-4 sm:py-6 flex items-center justify-between shrink-0 bg-white/80 dark:bg-[#161B2E]/80 backdrop-blur-md">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="sm:hidden p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-all active:scale-90"
                >
                  <ChevronLeft size={20} className="text-gray-400" />
                </button>
                <h2 className="text-2xl font-black tracking-tight text-maintext">
                  {searchQuery ? 'Search Results' : tabs.find(t => t.id === activeTab)?.label}
                </h2>
              </div>
              <button
                onClick={onClose}
                className="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-xl transition-all group active:scale-90"
              >
                <X
                  size={24}
                  className="text-gray-400 group-hover:text-maintext transition-colors"
                />
              </button>
            </div>
          ...
          >
            {deleteStep === 'confirm' && (
              <>
                ...
                <p className="text-sm text-subtext mb-8 leading-relaxed">
                  Are you absolutely sure? This will permanently remove your profile, data, and
                  access. <strong className="text-red-500">This cannot be undone.</strong>
                </p>
            ...
            {deleteStep === 'otp' && (
              <>
                ...
                <p className="text-sm text-subtext mb-6 leading-relaxed">
                  ...
                  <strong className="text-maintext">{user?.email}</strong>. Please verify it below.
                </p>
            ...
            {deleteStep === 'verified' && (
              <>
                ...
                <p className="text-sm text-subtext mb-8 leading-relaxed">
                  Your identity is verified. Ready to permanently remove all your profile data and
                  access.
                </p>
                <div className="flex flex-col gap-3">
                  <button
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full p-4 bg-gray-50 dark:bg-white/5 border border-border rounded-2xl outline-none focus:border-primary transition-all text-sm"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 px-1">
                ...
                <button
                  ...
                  className="w-full py-2 text-xs font-bold text-subtext hover:text-primary transition-colors"
                >
                  Resend Code
                </button>
              </div>
      ...
      {showCropper && (
        ...
        >
          <motion.div
            ...
            className="bg-white dark:bg-[#1E2438] rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col"
            onClick={e => e.stopPropagation()}
          >
