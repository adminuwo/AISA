    ...
    <div className={`relative w-full ${isOpen ? 'z-[50]' : 'z-[1]'}`} ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-3">
          {Icon && (
            <Icon
              className={`w-4 h-4 transition-colors ${isOpen ? 'text-primary' : 'text-gray-400 group-hover:text-primary'}`}
            />
          )}
          <span className="text-sm font-bold text-gray-700 dark:text-maintext">{value}</span>
        </div>
        <ChevronDown
          className={`w-4 h-4 text-gray-400 transition-transform duration-500 ${isOpen ? 'rotate-180 text-primary' : ''}`}
        />
      </button>
