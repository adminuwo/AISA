...
const ActionButton = ({ action, isSecondary = false, theme, onRestore }) => (
  ...
  >
    <action.icon
      className={`${isSecondary ? 'w-3.5 h-3.5' : 'w-4 h-4'} transition-transform group-hover:scale-110`}
    />
    <span
      className={`${isSecondary ? 'text-[11px] font-medium' : 'text-[12px] font-bold'} uppercase tracking-wider`}
    >
      {action.label}
    </span>
  </button>
...
const SelectionToolbar = ({ selection, onClose, onAction, onRestoreSelection, theme = 'dark' }) => {
    ...
    const updatePosition = () => {
      ...
      if (left < 20 || left + toolbarWidth > window.innerWidth - 20) {
        ...
        if (left < halfWidth + 20) left = halfWidth + 20;
          ...
          left = window.innerWidth - halfWidth - 20;
      }
  ...
  const primaryActions = [
    ...
    { id: 'ask', icon: Sparkles, label: 'Ask AI', onClick: e => onAction('ask', selection.text) },
    {
      ...
      icon: MessageCircle,
      label: 'Explain',
      onClick: e => onAction('explain', selection.text),
    },
    {
      id: 'translate',
      icon: Languages,
      label: 'Translate',
      onClick: e => onAction('translate', selection.text),
    },
    {
      id: 'search',
      icon: Search,
      label: 'Search Web',
      onClick: e => onAction('search', selection.text),
    },
  ];
  ...
  const secondaryActions = [
    ...
    ...(selection.isCode
      ? [{ id: 'copy-code', icon: FileCode, label: 'Copy Code', onClick: handleCopy }]
      : []),
    {
      id: 'highlight',
      icon: Highlighter,
      label: 'Highlight',
      onClick: e => onAction('highlight', selection.text),
    },
    ...
    { id: 'share', icon: Share2, label: 'Share', onClick: e => onAction('share', selection.text) },
    {
      ...
      icon: Download,
      label: 'Download TXT',
      onClick: e => onAction('download', selection.text),
    },
    { id: 'pdf', icon: FileText, label: 'Save PDF', onClick: e => onAction('pdf', selection.text) },
    {
      ...
      icon: FileCode,
      label: 'Summarize',
      onClick: e => onAction('summarize', selection.text),
    },
    {
      id: 'rewrite',
      icon: Edit3,
      label: 'Rewrite',
      onClick: e => onAction('rewrite', selection.text),
    },
    {
      id: 'bookmark',
      icon: Bookmark,
      label: 'Bookmark',
      onClick: e => onAction('bookmark', selection.text),
    },
  ];
    ...
    <AnimatePresence>
      <motion.div
        ...
        style={{
          ...
          left: position.left,
          transform:
            position.arrow === 'bottom' || position.arrow === 'top' ? 'translateX(-50%)' : 'none',
        }}
      >
        <div
          className={`
        ...
        >
          <div className="flex flex-col p-1.5 overflow-y-auto custom-scrollbar overflow-x-hidden">
            ...
            <div
              className={`my-1.5 h-px w-full ${theme === 'dark' ? 'bg-white/5' : 'bg-zinc-100'}`}
            />

            <button
              onMouseDown={e => {
                e.preventDefault();
                if (onRestoreSelection) onRestoreSelection();
              }}
              onTouchEnd={e => {
                e.preventDefault();
                e.stopPropagation();
