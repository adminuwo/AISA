...
export const PersonalizationProvider = ({ children }) => {
  ...
  const addNotification = notif => {
    ...
    const isDuplicate = notifications.some(
      n => n.title === notif.title && new Date(n.time).toDateString() === today
    );
    ...
    toast.custom(
      tObj => (
        <div
          className={`${tObj.visible ? 'animate-enter' : 'animate-leave'} max-w-sm w-full bg-white dark:bg-zinc-900 shadow-2xl rounded-2xl pointer-events-auto flex ring-1 ring-black ring-opacity-5 border border-indigo-500/20 overflow-hidden`}
        >
          <div className="flex-1 w-0 p-4">
            <div className="flex items-start">
              <div className="ml-0 flex-1">
                ...
                <p className="mt-1 text-[10px] text-gray-500 dark:text-gray-400">{notif.desc}</p>
              </div>
            </div>
          </div>
    ...
    if (user?.token) {
      ...
      socket.on('new_notification', notification => {
        ...
        toast.custom(
          tObj => (
            <div
              className={`${tObj.visible ? 'animate-enter' : 'animate-leave'} max-w-sm w-full bg-white dark:bg-[#1E1E1E] shadow-2xl rounded-2xl pointer-events-auto flex ring-1 ring-black ring-opacity-5 border border-primary/20 overflow-hidden`}
            >
              <div className="flex-1 w-0 p-4">
                <div className="flex items-start">
                  <div className="ml-0 flex-1">
          ...
          try {
            ...
            if (reminder.repeat === 'none') {
              const res = await axios.patch(
                `${API}/reminders/${reminder._id}/complete`,
                {},
                {
                  headers: { Authorization: `Bearer ${user.token}` },
                }
  ...
  const resetPersonalizations = async () => {
    try {
      if (user?.token) {
        await axios.post(
          apis.user + '/personalizations/reset',
          {},
          {
            headers: { Authorization: `Bearer ${user.token}` },
          }
  ...
  const getSystemPromptExtensions = () => {
    ...
    if (p.emojiUsage) {
      const map = {
        ...
        Expressive: 'Use emojis frequently.',
      };
