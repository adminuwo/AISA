  ...
  const sections = [
    {
      ...
      items: [t('tos_accept_item1'), t('tos_accept_item2'), t('tos_accept_item3')],
    },
    {
      icon: Scale,
      title: t('tos_use_title'),
      items: [t('tos_use_item1'), t('tos_use_item2'), t('tos_use_item3'), t('tos_use_item4')],
    },
    {
      icon: DollarSign,
    ...
    },
    {
      ...
      items: [t('tos_ip_item1'), t('tos_ip_item2'), t('tos_ip_item3'), t('tos_ip_item4')],
    },
    {
      icon: AlertCircle,
      ...
      {dynamicSections ? (
        <>
          {dynamicSections.map((section, index) => (
            <div
              key={index}
              className="bg-surface/30 rounded-[2rem] sm:rounded-2xl p-5 sm:p-8 border border-border hover:border-primary/20 transition-all group"
            >
              <div className="flex items-start gap-3 sm:gap-4 mb-5 sm:mb-8">
                ...
                <div>
                  <h3 className="text-lg sm:text-xl font-bold text-maintext pt-1.5 sm:pt-2">
                    {section.title}
                  </h3>
              ...
              <div className="space-y-6 sm:space-y-10">
                {section.content.map((item, idx) => (
                  <div key={idx} className="space-y-3 sm:space-y-4">
                    {item.subtitle &&
                      !['General Terms', 'Policy Overview', 'Introduction', 'N/A', ''].includes(
                        item.subtitle
                      ) && (
        ...
        <>
          <div className="bg-white dark:bg-white/[0.04] rounded-xl p-4 border border-border">
            <p className="text-sm text-maintext leading-relaxed">{t('tos_intro')}</p>
          </div>

          {sections.map((section, index) => (
            <div
              key={index}
              className="bg-white dark:bg-white/[0.04] rounded-xl p-5 border border-border hover:border-primary/30 transition-all"
            >
      ...
      <div className="bg-gradient-to-r from-primary/5 to-blue-500/5 rounded-xl p-5 border border-primary/20">
        ...
        <div className="space-y-1.5 text-sm text-subtext">
          ...
          <p>
            ...
            <a href="tel:+918358990909" className="text-primary hover:underline">
              +91 83589 90909
            </a>
          </p>
        </div>
