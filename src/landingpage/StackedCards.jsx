...
const LabCard = ({ title, subtitle, icon: Icon, color, slides, onClick }) => {
  ...
  return (
    ...
    >
      ...
      {/* Glass Background */}
      <div
        className={`absolute inset-0 backdrop-blur-[40px] rounded-[19px] transition-colors duration-500 bg-[#0a0a12]/90`}
      />
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-1000 ${isInView ? 'opacity-30' : 'opacity-0'} group-hover:opacity-100`}
      >
        <div
      ...
      <div className="relative h-full w-full rounded-[19px] p-8 flex flex-col items-center z-10 transition-all duration-500 border border-white/5 group-hover:border-white/10 bg-[#0a0a12]/95">
        {/* Icon Section */}
            ...
            {isInView && (
              ...
              >
                  ...
                  <AnimatePresence mode="wait">
                    {currentSlide.type === 'intro' ? (
                      ...
                      >
                        ...
                        <h4 className="text-xl font-black mb-3 text-white tracking-tight italic uppercase">
                          {currentSlide.label}
                        </h4>
                        <p className="text-xs font-medium text-white/50">{currentSlide.text}</p>
                      </motion.div>
                    ) : currentSlide.type === 'upload' ? (
                      <motion.div key="upload" className="w-full flex flex-col items-center">
                      ...
                      <motion.div key="analysis" className="w-full flex flex-col items-center">
                        ...
                        <h4 className="text-sm font-black mb-1 text-white tracking-tight italic">
                          {currentSlide.label}
                        </h4>
                      ...
                      <div className="w-full space-y-3">
                        ...
                        >
                          ...
                          <span className="text-[8px] font-black text-red-400 bg-red-400/10 px-2 py-0.5 rounded shadow-[0_0_10px_rgba(239,68,68,0.2)]">
                            ⚠ Risk detected
                          </span>
                        </motion.div>
                        <motion.div
                        ...
                        >
                          ...
                          <span className="text-[8px] font-black text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded shadow-[0_0_10px_rgba(16,185,129,0.2)]">
                            ✅ Safe clause
                          </span>
                        </motion.div>
                        <h4 className="text-xs font-black text-white/40 uppercase tracking-[0.2em] mt-4">
                          {currentSlide.label}
                        </h4>
                      </div>
                    ) : (
                      ...
                      >
                        ...
                        <h4 className="text-base font-black mb-3 text-white italic uppercase tracking-tight">
                          {currentSlide.label}
                        </h4>
                        <p className="text-[11px] leading-relaxed text-white/60 mb-8 max-w-[220px]">
                          {currentSlide.text}
                        </p>
                ...
                >
                  ...
                  <ArrowRight
                    size={14}
                    className="relative z-10 transition-transform group-hover/btn:translate-x-1.5"
                    style={{ color }}
                  />
                </motion.button>
              </motion.div>
...
const StackedCards = () => {
  ...
  return (
    <section
      className={`relative py-32 px-8 overflow-hidden flex flex-col items-center justify-center transition-colors duration-700 bg-[#0b0b12]`}
    >
      {/* Aesthetic Neural Background / Subtle Gradients for Light Mode */}
      <div
        className={`absolute inset-x-[-100px] inset-y-0 pointer-events-none opacity-20 bg-[radial-gradient(circle_at_center,rgba(99,102,241,0.05)_0%,transparent_70%)]`}
      />
      ...
      <div className="text-center mb-20 z-30">
        <motion.div
          ...
          className={`inline-flex items-center gap-4 px-8 py-3 rounded-full mb-8 backdrop-blur-3xl border transition-colors bg-white/[0.04] border-white/10`}
        >
          <Sparkles size={14} className="text-primary animate-pulse" />
          <span
            className={`text-[10px] font-black tracking-[0.8em] uppercase leading-none mt-1 text-white/50`}
          >
            Specialized Lab Nodes
          </span>
        </motion.div>
        <h2 className="text-6xl font-black italic tracking-tighter uppercase mb-4 text-white leading-none">
          Two Core Wings of AISA™
        </h2>
        <p className="text-base font-medium tracking-tight text-gray-500">
          Unified Intelligence & Frontier Generative Engines.
        </p>
      </div>
