...
const SlideShow = ({ type, themeColor, isDarkMode }) => {
        ...
        <div className="w-full h-full flex flex-col p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 h-full pb-4">
            {[
              ...
              >
                <div
                  className="w-8 h-8 rounded-lg mb-2 flex items-center justify-center font-black text-[10px]"
                  style={{ background: stock.c + '44', color: stock.c }}
                >
                  {stock.s.slice(0, 2)}
                </div>
        ...
        <div className="w-full h-full flex items-center justify-center p-6">
          ...
          >
            ...
            <div className="flex gap-2">
              <div className="px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-[8px] font-black text-white/60">
                SOFTWARE
              </div>
              <div className="px-2 py-0.5 rounded-md bg-blue-500/20 border border-blue-500/40 text-[8px] font-black text-blue-400">
                US
              </div>
            </div>
            <motion.div
      ...
      visual: (
        <div className="w-full h-full flex flex-col justify-center items-center gap-6 p-8">
          ...
          <div className="text-center">
            <div className="text-[9px] font-black text-white/20 tracking-[0.4em] mb-1">
              REAL-TIME DATA FEED
            </div>
        ...
        <div className="w-full h-full flex flex-col justify-center p-8 space-y-4">
          <div className="bg-white/5 rounded-2xl p-4 border border-white/10 relative">
            ...
            <div className="space-y-2">
              ...
              >
                "Transforming your workspace has never been easier. Discover premium aesthetics with
                AISA...{' '}
                <motion.span animate={{ opacity: [0, 1] }} transition={{ repeat: Infinity }}>
                  |
                </motion.span>
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {['#Productivity', '#AIArt', '#ModernWork'].map(tag => (
                  <span
                    key={tag}
                    className="text-[9px] font-black text-primary/40 bg-primary/5 px-2 py-0.5 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
  ...
  return (
    <div className="flex flex-col h-full">
      <div
        className={`flex-1 relative overflow-hidden rounded-2xl border mb-6 shadow-inner ${isDarkMode ? 'bg-black/20 border-white/5' : 'bg-[#0f111a] border-slate-200/50'}`}
      >
        ...
        <motion.div
          ...
          style={{
            ...
            background:
              state === 'opened'
                ? isDarkMode
                  ? 'linear-gradient(160deg, rgba(30,30,50,0.98), rgba(10,10,20,1))'
                  : 'linear-gradient(160deg, rgba(255,255,255,1), rgba(245,247,250,1))'
                : isDarkMode
                  ? `linear-gradient(160deg, ${themeColor}15, rgba(11,11,18,0.98))`
                  : `linear-gradient(160deg, rgba(255,255,255,0.95), rgba(240,245,255,0.9))`,
            boxShadow:
              state === 'opened'
                ? isDarkMode
                ...
                : isDarkMode
                  ? `0 30px 100px ${themeColor}33, 0 0 40px ${themeColor}15, inset 0 0 20px ${themeColor}08`
                  : `0 25px 50px ${themeColor}22, inset 0 0 20px rgba(255,255,255,1)`,
            border:
              state === 'opened'
                ? isDarkMode
        ...
        >
          ...
          {state === 'closed' && (
            <div
              ...
              style={{ boxShadow: `0 0 60px 20px ${themeColor}44` }}
            />
          )}
          ...
          >
            {state === 'closed' || state === 'impact' ? (
              <div className="flex flex-col items-center">
                ...
                <motion.div
                  animate={
                    state === 'impact' ? { scale: [1, 0.9, 1.15], rotate: [0, -5, 5, 0] } : {}
                  }
                  ...
                  <AnimatePresence>
                    {state === 'impact' && (
                      <motion.div
                        ...
                        style={{
                          background: `radial-gradient(circle, ${themeColor} 100%, transparent)`,
                        }}
                      />
                    )}
                  </AnimatePresence>
                ...
                <div className="text-center relative z-10">
                  <h3
                    className={`text-4xl font-black mb-3 tracking-tighter ${isDarkMode ? 'text-white' : 'text-slate-800'}`}
                    style={{ textShadow: isDarkMode ? `0 0 30px ${themeColor}44` : 'none' }}
                  >
                    {title}
                  </h3>
                  <motion.div
                  ...
                  >
                    <span
                      className={`text-[12px] font-black uppercase tracking-[0.3em] transition-all group-hover/btn:tracking-[0.4em] ${isDarkMode ? 'text-white/40' : 'text-slate-500'}`}
                    >
                      Tap to Open
                    </span>
                    <div
                      className="w-2.5 h-2.5 rounded-full animate-heartbeat"
              ...
              >
                ...
                <div className="flex-1 min-h-[250px] sm:min-h-[300px]">
                  <SlideShow
                    type={isCashflow ? 'cashflow' : 'aiadd'}
                ...
                <div className="w-full md:w-[350px] flex flex-col justify-center items-center md:items-start text-center md:text-left pt-4 md:pt-0 pb-6 md:pb-0">
                  ...
                  <h2
                    className={`text-3xl sm:text-5xl font-black mb-2 sm:mb-4 tracking-tight leading-[1] ${isDarkMode ? 'text-white' : 'text-slate-800'}`}
                  >
                    Launching <br /> <span className="text-primary italic">Soon 🚀</span>
                  </h2>

                  <p
                    className={`text-sm sm:text-lg mb-6 sm:mb-10 font-medium leading-relaxed ${isDarkMode ? 'text-white/40' : 'text-slate-500'}`}
                  >
                  ...
                  <div className="space-y-3 w-full">
                    ...
                    <p
                      className={`text-[9px] font-black uppercase tracking-[0.3em] text-center ${isDarkMode ? 'text-white/20' : 'text-slate-400'}`}
                    >
...
const ComingSoonSurprise = () => {
  ...
  return (
    <section
      className={`relative py-32 px-6 overflow-hidden flex flex-col items-center justify-center transition-colors duration-500 ${isDarkMode ? 'bg-[#0b0b12]' : 'bg-[#f8fafc]'}`}
    >
      {/* Background Ambience */}
      <div
        className={`absolute top-1/4 left-1/4 w-[800px] h-[800px] rounded-full blur-[180px] pointer-events-none ${isDarkMode ? 'bg-primary/5' : 'bg-primary/10'}`}
      />
      <div
        className={`absolute bottom-1/4 right-1/4 w-[800px] h-[800px] rounded-full blur-[180px] pointer-events-none ${isDarkMode ? 'bg-purple-500/5' : 'bg-purple-500/10'}`}
      />
      ...
      <div className="text-center mb-24 relative z-10">
        ...
        >
          ...
          <span
            className={`text-[11px] font-black uppercase tracking-[0.3em] ${isDarkMode ? 'text-white/40' : 'text-slate-500'}`}
          >
            Expansion Lab 002
          </span>
        </motion.div>
        ...
        >
          Unlock the blueprints of our next frontier tools. Opening these nodes reveals the future
          of AISA intelligence.
        </motion.p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-12 lg:gap-24 relative z-10">
        <GiftBox
          ...
          isDarkMode={isDarkMode}
        />
        <GiftBox title="AIADD" themeColor="#a855f7" isCashflow={false} isDarkMode={isDarkMode} />
      </div>
