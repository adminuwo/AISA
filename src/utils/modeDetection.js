...
export function detectMode(message = '', attachments = []) {
  ...
  if (attachments && attachments.length > 0) {
    const hasImages = attachments.some(
      a => (a.type && a.type.startsWith('image')) || (a.mimeType && a.mimeType.startsWith('image'))
    );
  ...
  const hasWritingKeywords = WRITING_KEYWORDS.some(keyword => lowerMessage.includes(keyword));

  if (hasWritingKeywords) {
    return MODES.CONTENT_WRITING;
  }

  const hasTaskKeywords = TASK_KEYWORDS.some(keyword => lowerMessage.includes(keyword));
